...
class JsonProvider(object):
    def __init__(self, rpc_addr, proxies=None, headers=None):
    ...
    def json_rpc(self, method: str, params: Union[dict, list, str], timeout: 'TimeoutType' = 2.0) -> dict:
        ...
        r = requests.post(self.rpc_addr(), json=j, timeout=timeout, proxies=self.proxies, headers=self.headers)
        r.raise_for_status()
    ...
    def get_status(self, timeout: 'TimeoutType' = 2.0) -> dict:
        r = requests.get("%s/status" % self.rpc_addr(), timeout=timeout, proxies=self.proxies)
        r.raise_for_status()
