...
class AuthenticationManagerImpl:
    ...
    def get_access_token_or_authenticate(self, insecure) -> Optional[str]:
        if not self._access_token:
            self._access_token = self._read_token_from_file()
            if not self._access_token:
                if not self._services:
                    self._auth_config = self._load_auth_config(insecure)
    ...
    @staticmethod
    def _load_auth_config(insecure) -> Optional[OktaAuthConfig]:
        config = ff.Client(insecure=insecure).get_auth_config()
