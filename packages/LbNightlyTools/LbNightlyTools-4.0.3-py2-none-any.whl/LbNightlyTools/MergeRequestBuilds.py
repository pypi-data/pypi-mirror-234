...
def get_model_slot(target_branch, slots):
    ...
    message = 'Cannot determine Slot configuration from target branch "{}".'.format(
        target_branch
    )
...
def create_mr_slots(sources, platforms, merge, model_slots, with_downstream):
    """
    ...
    ref_slot = clone_slot(model_slot, model_name + "-ref", "MR reference slot", None)
    ...
    for name, ss in list(project_sources.items()):
        if name not in ref_repos:
            if ss[0].project_path.startswith("lhcb-datapkg/"):
                ...
                ref_slot.projects["DBASE"].packages.append(Package(name, target_branch))
            else:
                logger.warning(
