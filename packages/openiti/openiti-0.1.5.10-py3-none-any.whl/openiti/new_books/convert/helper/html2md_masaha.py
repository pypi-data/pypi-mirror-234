"""Convert Masaha library html to OpenITI mARkdown.
...
The subclass in this module, MasahaConverter,
adds methods specifically for the conversion of books from
the Masaha library to OpenITI mARkdown:
...
of the MasahaConverter class and return the converted string.

Examples (doctests):

    Headings: h1 (from superclass html2md.MarkdownConverter)

    >>> import html2md_masaha
    >>> h = '<h1>abc</h1>'
    >>> html2md_masaha.markdownify(h)
    '\\n\\n### | abc\\n\\n'
    ...
    >>> h = '<h1>abc</h1>'
    >>> html2md_masaha.markdownify(h, md_style=UNDERLINED)
    '\\n\\nabc\\n===\\n\\n'

    >>> h = '<h1>abc</h1>'
    >>> html2md_masaha.markdownify(h, md_style=ATX)
    '\\n\\n# abc\\n\\n'

    Headings: <h4>
    
    NB: in the Masaha library, <h4> tag is used for section headings
...
class MasahaConverter(html2md.MarkdownConverter):
    """Convert Masaha library html to OpenITI mARkdown."""
...
##        return text
    ...
    def convert_p(self, el, text):
        ...
        Example:
            ...
            >>> h = 'abc <p class="pagen">الصفحة 11</p> jkl'
            >>> html2md_masaha.markdownify(h)
            'abc \nPageV01P011\njkl'

            >>> h = '<p class="sher">abc * * * def</p>'
            >>> html2md_masaha.markdownify(h)
            '\n# abc %~% def\n'
...
def markdownify(html, **options):
    """Shortcut to the convert method of the MasahaConverter class."""
    return MasahaConverter(**options).convert(html)
