...
def get_valuation(security, start_date=None, end_date=None, count=None, fields=None):
    """
    获取多个标的在指定交易日范围内的市值表数据

    Args:
        security: 一支证券代码或者一个证券代码的list
        start_date: 与 count 二选一，不可同时使用. 格式为20231009的数字或者 datetime.datetime/datetime.date 对象, 开始时间
        end_date: 格式同上, 结束时间
