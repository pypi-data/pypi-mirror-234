...
async def test_search_handles_id_in_query(api, store_response):
    ...
    for _ in range(2):
        ...
        assert (await results[0].summary()) == (
            'Jake Blues, just released from prison, puts his old band back togther '
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        (Query('Deadwood', year=2004), ''),
        (Query('Le dolci signore', year=1967), 'Anyone Can Play'),
    ),
    ids=lambda value: str(value),
)
@pytest.mark.asyncio
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        (Query('Anyone Can Play', year=1967), 'Le dolci signore'),
    ),
    ids=lambda value: str(value),
)
@pytest.mark.asyncio
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        # The Blues Brothers
        ('movie/525', 'http://themoviedb.org/t/p/w300_and_h450_bestv2/lpmMVXDqfEcMaqTRbAcGYZ4hYVg.jpg'),
