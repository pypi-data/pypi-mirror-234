...
def ListOf(item_type, iterable=()):
    ...
    class ListOf(collections.abc.Sequence):
        ...
        def __str__(self):
            # FIXME: Ideally, types.Regex() should return a type with a
            #        __str__() method that returns the regular expression as a
            #        normal string (i.e. re.Pattern.pattern). But
            #        re.Pattern.__str__() returns "re.compile(r'....')" instead,
            #        which shouldn't be exposed to the user. re.Pattern cannot
            #        be subclassed, so we use this special case to list regular
            #        expressions nicely.
            def stringify(item):
