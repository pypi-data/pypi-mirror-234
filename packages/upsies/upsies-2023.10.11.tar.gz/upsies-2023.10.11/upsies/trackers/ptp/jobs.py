...
class PtpTrackerJobs(TrackerJobsBase):
    ...
    async def autodetect_trumpable(self, job):
        ...
        if self.options.get('no_english_subtitles', None) in (True, False):
            ...
            #   - Unknown language is considered non-English
            audio_languages = utils.video.languages(self.content_path).get('Audio', ())
            _log.debug('Trumpable reasons: Autodetected audio languages: %r', audio_languages)

            if 'en' not in audio_languages:
                subtitle_languages = {
                    subtitle.language
                    for subtitle in self.subtitles
                }
                _log.debug('Trumpable reasons: Autodetected subtitle languages: %r', subtitle_languages)
