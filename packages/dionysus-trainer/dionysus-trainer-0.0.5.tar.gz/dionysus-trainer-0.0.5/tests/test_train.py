...
class Test(unittest.TestCase):      
    def test_training(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert subdirs[1].endswith("ffw_moon"), f"results directory: {subdirs[1]} was no created"
            assert "info.log" in os.listdir(subdirs[1]), "logfile was not created"
            assert "cm.png" in os.listdir(subdirs[1]), "classification matrix plot was not created"
            assert "loss.png" in os.listdir(subdirs[1]), "loss plot was not created"
            assert "training_metrics.png" in os.listdir(subdirs[1]), "training metrics plot was not created"
            assert "validation_metrics.png" in os.listdir(subdirs[1]), "validation metrics plot was not created"

            assert subdirs[2].endswith("ffw_moon/last"), f"results last directory: {subdirs[2]} was no created"
