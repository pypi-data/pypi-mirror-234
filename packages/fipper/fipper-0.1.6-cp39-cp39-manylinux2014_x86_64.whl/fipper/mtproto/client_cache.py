...
from fipper.xdcalls import CacheCalls
from fipper.xdcalls.groups import GroupCallParticipant
from fipper.xdcalls.participant_list import ParticipantList

from .bridged_client import BridgedClient

py_logger = logging.getLogger('fipper')
...
class ClientCache:
    def __init__(
        ...
        self.assistant: BridgedClient = app
        self._cache_duration = cache_duration
        self._full_chat_cache = CacheCalls()
        self._call_participants_cache = CacheCalls()
    ...
    async def get_full_chat(
        ...
        if full_chat is not None:
            ...
            try:
                ...
                full_chat = await self.assistant.get_call(chat_id)
                self.set_cache(
                    chat_id,
                    full_chat,
                )
        ...
        if input_call is not None:
            ...
            if participants is not None:
                ...
                if not (last_update - curr_time > 0):
                    ...
                    try:
                        list_participants = await self.assistant.get_participants(
                            input_call,
                        )
