...
def nested_call_resouce(path: str, action: str):
    def inner(cls):
        def nested_url(cls):
            ...
            if action == "create":
                return f"call/{path}", "POST"
            elif action == "modify":
                return f"call/{path}", "PUT"
            elif action == "delete":
                return f"call/{path}", "DELETE"
            elif action == "get":
                return f"call/{path}", "GET"
        ...
        def nested_create_resouce(cls, *args, **kwargs):
            ...
            return conv.convert_to_anonpi(cls, res.prepare_request(res), clsname="recording" if "record" in path else "call")
