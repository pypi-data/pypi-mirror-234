...
class AnonObject(AnonBaseObject):
    @classmethod
    def create(cls, from_number: str, to_number: str, callback_url: str, amd: bool = False, **params) -> t.Self:
        ...
        Raises:
            ...
            ... Futher Exceptions
        """
        def creating_call(*args, **kwargs):
            res = RequestCooker.cook_request(cls, "_make_create", {
                "from_number": from_number, "to_number": to_number, "callback_url": callback_url, "amd": amd, **params})
            return conv.convert_to_anonpi(cls, res.prepare_request(res), clsname="call")
        return creating_call(cls, from_number=from_number, to_number=to_number,
                             callback_url=callback_url, amd=amd, **params)
