...
class _Nator:
    ...
    def generate(self, options: List[str], user_options: List[str] = None) -> Dict:
        if user_options:
        ...
        return self.request(f"generate-{self.inbox_type}", json_data)

    def messages_list(self, inbox: str) -> List[Dict]:
        json_data = {self.inbox_type: inbox}

        response = self.request("message-list", json_data)
...
class EmailNator(_Nator):
    ...
    def get_messages(self, email: str) -> List[Email]:
        messages = self.messages_list(email)
        return [
            Email(
                message["messageID"],
                message["from"],
...
class SMSNator(_Nator):
    ...
    def generate_number(self, options: List[str] = None) -> str:
        response = self.generate(["SE", "GB", "FR", "FI", "DK", "BE"], options)
        return response["number"]

    def get_messages(self, number: str) -> List[SMS]:
        messages = self.messages_list(number)
        return [
            SMS(
                message["messageID"],
                message["from"],
