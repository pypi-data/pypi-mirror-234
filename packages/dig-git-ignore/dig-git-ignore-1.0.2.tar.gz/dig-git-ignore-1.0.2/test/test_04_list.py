...
def test_list_without_existing_gitignore() -> None:
    os.remove(constants.GITIGNORE_FILENAME)
    main._main(["list"])


def test_list_with_existing_gitignore_without_existing_template() -> None:
    with open(constants.GITIGNORE_FILENAME, mode="wt", encoding="utf-8") as file:
        file.write("# Non empty file")
    main._main(["list"])


def test_list_with_existing_gitignore_with_existing_template() -> None:
