...
def _extract_node_templates_from_function_graph(fn_graph: graph.FunctionGraph) -> List[dict]:
    ...
    for node_ in fn_graph.nodes.values():
        code_artifact_pointers = [] if (node_.originating_functions is None or len(
            node_.originating_functions
        ) == 0) else [_get_fully_qualified_function_path(fn) for fn in node_.originating_functions]
...
class DAGWorksGraphExecutor(driver.GraphExecutor):
    ...
    def _register_or_query(self, fg: graph.FunctionGraph) -> int:
        ...
        dag_template_id = self.client.register_dag_template_if_not_exists(
            ...
            code_artifacts=extract_code_artifacts_from_function_graph(fg, self.vcs_info, self.repo_base),
            config=fg.config,
            tags=self.run_tags,
