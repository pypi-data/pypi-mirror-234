...
def get_ztf_detections(
    ...
    """
    Calculates the number of detections per band above an SNR threshold. Calls :func:`get_ztf_lightcurve` to get the
    lightcurve.
    ...
    database_connector = DatabaseConnector(base_name=base_name, database_name=database_name)
    peak_times = database_connector.get_peak_time_jd(index=_index.astype(int)).median(axis=1)
    excess_times = database_connector.get_excess_mjd(index=_index.astype(int))
    start_times = excess_times[["W1_excess_start_mjd", "W2_excess_start_mjd"]].median(axis=1)

    detected = dict()

    for i in tqdm(_index, desc="going through forced photometry"):
        i_peak_time = Time(peak_times.loc[str(i)], format="jd")
        i_start_time = Time(start_times.loc[str(i)], format="jd")
        ...
        lc_times = lc["obsmjd"]
        time_mask = (
                (lc_times >= i_start_time.mjd + time_interval_relative_to_peak[0]) &
        ...
        for f in ["g", "r", "i"]:
            if lc is not None:
                ...
                idetected[f] = np.sum(lc["above_snr"][bandmask] & time_mask)
                idetected[f + "_all"] = np.sum(bandmask & time_mask)
...
def ztf_detected(
    ...
    one band for at least `n_detections` times in the time interval relative to the IR peak. See
    :func:`get_ztf_detections` for more details.
    ...
    ztf_detections = get_ztf_detections(
        base_name=base_name,
        database_name=database_name,
        index=index,
        snr_threshold=snr_threshold,
        time_interval_relative_to_peak=time_interval_relative_to_peak
    )

    return ztf_detections[["g", "r", "i"]] >= n_detections
