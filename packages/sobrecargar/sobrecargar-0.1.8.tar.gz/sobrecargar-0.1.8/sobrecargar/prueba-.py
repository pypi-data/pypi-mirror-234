...
if __name__ == '__main__': 
    ...
    @sobrecargar
    def unaFunc(x: int, y : str, *posicionales):
        print(f"x es int : {x} | y es str : {y}")
        for a in posicionales:
            print(f"a es {type(a)} : {a}")



    @sobrecargar
    def unaFunc(x: int, y : str, z : bool):
        print(f"x es int : {x} | y es str : {y} | z es bool {z}")


    unaFunc(5,"Hola",True)
    unaFunc(19,"chau",6,7,8,9)
