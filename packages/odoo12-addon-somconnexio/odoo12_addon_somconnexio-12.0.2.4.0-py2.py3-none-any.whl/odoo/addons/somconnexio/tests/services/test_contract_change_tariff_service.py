...
class TestContractChangeTariffService(BaseEMCRestCaseAdmin):
    ...
    def test_run_wizard_quit_sharing_tariff(
        self, mock_quit_sharing_bond
            ):
        contract = self.browse_ref('somconnexio.contract_mobile_il_50_shared_1_of_3')
        sharing_contract = self.browse_ref(
            'somconnexio.contract_mobile_il_50_shared_2_of_3')
        self.assertEqual(len(contract.sharing_bond_contract_ids), 3)
        self.assertIn(contract, sharing_contract.sharing_bond_contract_ids)
        self.assertEqual(
            sharing_contract.current_tariff_product,
