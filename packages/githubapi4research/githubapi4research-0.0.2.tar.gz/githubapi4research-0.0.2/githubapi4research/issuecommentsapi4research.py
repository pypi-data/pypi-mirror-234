...
class IssueCommentsAPI4Research(GithubAPI4Research):
    def get(self, start_time=None, end_time=None, checkpoint=200, author=None):
        ...
        if os.path.exists(f"{self.to_dir}/{self.repo_name}IssueCommentsIndexTmp.log"):
            with open(f"{self.to_dir}/{self.repo_name}IssueCommentsIndexTmp.log") as fp:
                line = fp.readline()
        ...
        while True:
            try:
                url = "https://api.github.com/repos/{}/{}/issues/comments?&per_page=100&page={}&sort=created&direction=asc&since={}".format(self.repo_owner, self.repo_name, index, sinceTime)
                print(url)
                response = requests.get(url=url, headers={'Authorization': 'token {}'.format(self.api_token)}, verify=False)
                ...
                if response.status_code == requests.codes.ok:
                    ...
                    if len(response.json()) > 0:
                        ...
                        print("Got 100 IssueComments from page {}".format(index)) 
                        index += 1
                    else:
                        print("Get all IssueComments successfully") 
                        break
                else:
                    print("Stop to get IssueComments due to {}".format(response.text))
                    break
            ...
            if index != 0 and index % checkpoint == 0:
                ...
                with open(f"{self.to_dir}/{self.repo_name}IssueCommentsIndexTmp.log", "w", encoding='utf-8') as fp:
        ...
        if os.path.exists(f"{self.to_dir}/{self.repo_name}IssueCommentsIndexTmp.log"):
            os.remove(f"{self.to_dir}/{self.repo_name}IssueCommentsIndexTmp.log")
