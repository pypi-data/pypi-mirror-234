...
class GenProcedure(ABC):
    def __init__(fileProviders:List[RepoObjectProvider], runners:List[Runner], loggersset:List[Logger], perScan:Union[BetweenScans, None] = None, stage:Union[str, None]=None):
    ...
    async def setLoggers(self):
        if len(self.loggerSet) == 0:
            for logger in loggerset:
                await self.loggerSet.add(logger)
    
    async def files(self) -> List[RepoObject]:
    ...
    def getRunnerProcedure(self, runner:Runner):
        class RunnerProcedure(object):
            ...
            async def __aenter__(self):
                await self.loggerSet.send("Entering the runner {0}".format(runner.name()))
                await self.runner.initialize()
                await self.loggerSet.send("Initialized the runner {0}".format(runner.name()))

            async def __aexit__(self, _type=None, value=None, traceback=None):
                await self.loggerSet.send("Exiting the runner {0}".format(runner.name()))
                await self.runner.clean()
                await self.loggerSet.send("Cleaned the runner {0}".format(runner.name()))

        return RunnerProcedure(runner, loggerSet=self.loggerSet)
