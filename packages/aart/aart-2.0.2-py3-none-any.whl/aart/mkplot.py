...
def bhimage(mask1, mask2, fig_size=[5,5], res=400, save_path='./Results/', **params):
    ...
    fint=load_path+"Intensity_a_%s_i_%s_sk_%s_bphi_%s_br_%s_bvapp_%s_gfact_%s_gp_%s_sp_%s.h5"%(spin_case,i_case, sub_kep, betaphi, betar,  bvapp, gfactor, gammap, sigmap)
    ...
    plt.savefig(save_path+'BHImage_a_%s_i_%s_sk_%s_bphi_%s_br_%s_bvapp_%s_gfact_%s_gp_%s_sp_%s.png'%(spin_case,i_case, sub_kep, betaphi, betar,  bvapp, gfactor, gammap, sigmap),dpi=res,bbox_inches='tight')

    plt.show()
    
def visplot(mask1, mask2, rangles=[0,90], fig_size=[5,5], res=400, save_path='./Results/', **params):
    """
    ...
    for i in range(len(rangles)):
        fnrays=load_path+"Visamp_%s_a_%s_i_%s_dx0_%s_dx1_%s_dx2_%s_sk_%s_bphi_%s_br_%s_bvapp_%s_gfact_%s_gp_%s_sp_%s_fudge_%s_Ncut_%s.h5"%(rangles[i],spin_case,i_case, dx0, dx1, dx2, sub_kep, betaphi, betar,  bvapp, gfactor, gammap, sigmap, fudge, int(Ncut))
    ...
    if not isExist:
        os.makedirs(save_path)
    plt.savefig(save_path+"Visplot_rangles_%s_%s_a_%s_i_%s_dx0_%s_dx1_%s_dx2_%s_sk_%s_bphi_%s_br_%s_bvapp_%s_gfact_%s_gp_%s_sp_%s_fudge_%s_Ncut_%s.png"%(rangles[0], rangles[-1] ,spin_case,i_case, dx0, dx1, dx2, sub_kep, betaphi, betar,  bvapp, gfactor, gammap, sigmap, fudge, int(Ncut)),dpi=res,bbox_inches='tight')
