...
from pydantic.types import StrictStr, StrictInt, SecretStr
...
class БромКлиент:
    class HTTPBearerAuth(AuthBase):
        ...
        def __init__(self, token: SecretStr, scheme_name: str = r'Bearer') -> NoReturn:
            self.__token = token
        ...
        def __call__(self, request: Request, /) -> Request:
            request.headers['Authorization'] = ' '.join([self.scheme, self.token.get_secret_value()])
            return request
    ...
    class HTTPAdapter(RequestsHTTPAdapter):
        ...
        def send(self, request, stream=False, timeout=None, verify=True, cert=None, proxies=None) -> Response:
            response = super().send(request=request, stream=stream, timeout=timeout,
    ...
    def __init__(self, публикация="", пользователь="", пароль="", строкаПодключения=""):
        ...
        session.mount("https://", self.HTTPAdapter())
        session.mount("http://", self.HTTPAdapter())
    ...
    @contextmanager
    def UseHttpAuth(self, auth: HTTPBearerAuth | HTTPBasicAuth, /) -> Self:
        old_auth = self.SoapКлиент.transport.session.auth
        self.SoapКлиент.transport.session.auth = auth
        try:
            yield self
        finally:
            self.SoapКлиент.transport.session.auth = old_auth
