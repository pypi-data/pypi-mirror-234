...
class Claim:
    ...
    def claim_number(self) -> str:
        """ Claim Number """
        return self.claim.claim_number
    
    @property
    def patient_control_number(self) -> str:
        """ Patient Control Number, frequently is the MRN (Medical Record Number) """
        patient_control_number = [r for r in self.references if r.qualifier == 'medical record identification mumber']
        assert len(patient_control_number) == 1

        return patient_control_number[0]
