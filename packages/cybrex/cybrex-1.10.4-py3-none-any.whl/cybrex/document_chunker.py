...
from unstructured.chunking.title import _split_elements_by_title_and_table, chunk_table_element
...
class DocumentChunker:
    def __init__(self, text_splitter, minimal_chunk_size: int = 192, add_metadata: bool = False):
    ...
    def to_chunks(self, source_document: SourceDocument) -> List[Chunk]:
        ...
        for section in list(soup.find_all('section')):
            for child in section.children:
                if child.name in {'header', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'} and child.text in BANNED_SECTIONS:
                    section.extract()
                break
