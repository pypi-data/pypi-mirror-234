...
from jjuke.modules import default, cycle
from jjuke.modules.unet.modules import conv_nd, weight_standardized_conv_nd, \
    RMSNorm, LayerNorm
...
class ResnetBlock(nn.Module):
    def __init__(self, unet_dim: int, dim, dim_out, *, time_emb_dim=None, groups=8):
        super().__init__()
...
class LinearAttention2D(nn.Module):
    def __init__(self, dim, heads = 4, dim_head = 32):
        ...
        self.norm = LayerNorm(dim) # for 2D
...
def linear_attention_nd(unet_dim, *args, **kwargs):
    """ Specify LinearAttention for general U-Net """
    return {1: LinearAttention1D, 2: LinearAttention2D}[unet_dim](*args, **kwargs)
...
def attention_nd(unet_dim, *args, **kwargs):
    """ Specify Attention for general U-Net """
    return {1: Attention1D, 2: Attention2D}[unet_dim](*args, **kwargs)