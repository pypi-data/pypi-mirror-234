...
def Upsample(dim, unet_dim: int, dim_out=None, pool=False):
    if pool:
...
def Downsample(dim, unet_dim: int, dim_out=None, pool=False):
    if pool:
        return nn.Sequential(
...
class RMSNorm(nn.Module):
    def __init__(self, dim, unet_dim: int):
        super().__init__()
...
class PreNorm(nn.Module):
    """ for 1D U-Net """
    def __init__(self, dim, fn, unet_dim: int):
        super().__init__()
        self.fn = fn
