...
from jjuke.modules.unet.modules import conv_nd, RandomOrLearnedSinusoidalPosEmb, \
    SinusoidalPosEmb, Residual, PreNorm, Downsample, Upsample
from jjuke.modules.unet.unet_modules import attention_nd, linear_attention_nd, \
    ResnetBlock
...
class UnetBase(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            dim (int): Base dimension for U-Net. (e.g. 16)
            init_dim (int): If given, use it as an initial dimension. Defaults to None.
            out_dim (int): If given, use it as an output dimension. Defaults to None.
            dim_mults (tuple): Elements to be multiplied by the dim. Defaults to (1, 2, 4, 8).
            full_attn (tuple or list): All False for 1D UNet. Defaults to (False, False, False, True) (2D)
        """
        super().__init__()
        ...
        for ind, ((dim_in, dim_out), layer_full_attn, layer_attn_heads, layer_attn_dim_head) in enumerate(
            ...
            self.downs.append(nn.ModuleList([
                ...
                Downsample(dim_in, dim_out) if not is_last else conv_nd(self.unet_dim, dim_in, dim_out, 3, padding=1)
            ]))
        ...
        for ind, ((dim_in, dim_out), layer_full_attn, layer_attn_heads, layer_attn_dim_head) in enumerate(
            ...
            self.ups.append(nn.ModuleList([
                ...
                Upsample(dim_out, dim_in) if not is_last else conv_nd(self.unet_dim, dim_out, dim_in, 3, padding=1)
            ]))
