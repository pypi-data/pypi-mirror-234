import sys
sys.path.append("/root/dev/jjuke/jjuke")
import argparse
import math
import os
import random
...
from diffusion import get_betas
from diffusion.ddim import DDIMSampler
from diffusion.ddpm import DDPMSampler, DDPMTrainer
from diffusion.karras import KarrasSampler
from unet.unet_base import UnetBase
from unet.ldm_unet import UNetModel

from jjuke.utils.ema import ema
from jjuke.utils.sched import LinearWarmup, WarmupScheduler
from jjuke.metrics.score import get_inception_and_fid_score


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--result_dir", type=str, default="/root/hdd1/unet_test")
...
def train(args, model: nn.Module, model_ema: nn.Module):
    ...
    trainer = DDPMTrainer(betas, model_mean_type="eps", model_var_type=args.model_var_type, loss_type="l2").cuda()
    sampler = KarrasSampler(
        betas,
        karras_num_timesteps=args.n_sample_steps,
        karras_sampler="heun",
        clip_denoised=True,
            ...
            if step % args.samples_per_steps == 0:
                ...
                with torch.no_grad():
                    ...
                    if args.rankzero:
                        ...
                        torch.save(state_dict, args.result_dir / "best.pth")
...
def eval(args, model: nn.Module, model_ema: nn.Module):
    ...
    # sampler = DDIMSampler(
    #     betas,
    #     ddim_num_timesteps=args.n_sample_steps,
    #     ddim_eta=0.,
    #     model_mean_type="eps",
    #     model_var_type=args.model_var_type,
    #     clip_denoised=True,
...
def main_worker(rank: int, args: argparse.Namespace):
    ...
    #     tf_layers=2, # Num of "Blocks" in UnetBasase
    ...
    if not args.eval:
        ...
        print("Evaluation...")
        ckpt = torch.load(args.result_dir / "best.pth", map_location="cpu")
        if args.ddp:
