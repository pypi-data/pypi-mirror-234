...
class TripAssumptions(BaseModel):
    ...
    """
    transport_factor: Optional[conlist(TransportFactor, max_items=12, min_items=0, unique_items=True)] = Field(None, description="Transport speed change coefficients.")
    capacity_factor: Optional[conlist(CapacityFactor, max_items=12, min_items=0, unique_items=True)] = Field(None, description="Transport capacity change coefficients. Change the `capacity` parameters for all boxes and `max_capacity` for the transport. ")
    ...
    feature_lifetimes: Optional[conlist(FeatureLifetime, max_items=1000, min_items=0, unique_items=True)] = Field(None, description="List of tag lifetimes. Affects only on compatibility Performer-Order and Transport-Location. ")
