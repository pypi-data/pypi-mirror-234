...
class BayesStopping(BaseEstimator, ClassifierMixin):
    """
    Bayesian dynamic stopping [1]_. Fits Gaussian distributions for target and non-target responses, and calculates a
    ...
    def __init__(self, estimator, segment_time, fs, method="bes0", cr=1.0, target_pf=0.05, target_pd=0.80):
        """
        Constructor for Bayes dynamic stopping.
    ...
    def fit(self, X, y):
        ...
        X = np.sum(self.estimator._cca.transform(X=X), axis=1)
        n_samples = X.shape[1]
...
class BetaStopping(BaseEstimator, ClassifierMixin):
    """
    Beta dynamic stopping [1]_. Fits a beta distribution to non-max (correlation+1)/2, and tests the probability of the
    maximum correlation to belong to that same distribution.
    ...
    def __init__(self, estimator, target_p=0.95):
        """
        Constructor for beta dynamic stopping.
    ...
    def fit(self, X, y):
        """
        The training procedure to fit the dynamic procedure on supervised EEG data. Note, BetaStopping itself does not
        require training, it only trains the estimator.
...
class MarginStopping(BaseEstimator, ClassifierMixin):
    """
    Margin dynamic stopping [1]_. Learns threshold margins (difference between best and second best score) to stop at
    ...
    def __init__(self, estimator, segment_time, fs, target_p=0.95, margin_min=0.0, margin_max=1.0, margin_step=0.05):
        """
        Constructor for margin dynamic stopping.
