...
class eCCA(BaseEstimator, ClassifierMixin):
    """
    The 'ERP CCA', which is the "reference" method [1]_ which computes ERPs as templates for full sequences and performs
    a CCA for spatial filtering.
    ...
    def __init__(self, lags, fs, cycle_size=None, template_metric="mean", score_metric="correlation", cca_channels=None,
        ...
        """
        Constructor for eCCA.

        Args:
            lags: np.ndarray
                A vector of latencies in seconds per class relative to the first code if codes are circularly shifted
                versions of the first code, or None if all codes are different or this circular shift feature should be
                ignored.
            fs: int
            ...
            score_metric: str (default: "correlation")
                Metric to use to compute the overlap of templates and single-trials during testing: correlation,
                euclidean, inner.
            cca_channels: list (default: None)
                A list of channel indexes that need to be included in the estimation of a spatial filter at the template
                side of the CCA, i.e. CCA(X, T[:, cca_channels, :]). If None is given, all channels are used.
            lx: float | list (default: None)
                Regularization on the covariance matrix for CCA for all or each individual parameter along X (channels).
                If None, no regularization is applied.
            ly: float | list (default: None)
                Regularization on the covariance matrix for CCA for all or each individual parameter along T (channels).
                If None, no regularization is applied.
            latency: np.ndarray (default: None)
                The raster latencies of each of the classes of shape (n_classes,) that the data/templates need to be
                corrected for.
    ...
    def decision_function(self, X):
        ...
        if self.ensemble:
            ...
            for i_class in range(T.shape[0]):
                ...
                Xi = np.sum(self._cca[i_class].transform(X=X), axis=1)
            ...
            # Apply spatial filter
            X = np.sum(self._cca.transform(X=X), axis=1)
    ...
    def fit(self, X, y):
        ...
        if self.ensemble:
            ...
            for i_class in range(n_classes):
                self.T_[i_class, :] = np.sum(self._cca[i_class].transform(X=T[i_class, :, :].T), axis=1)
        else:
            self.T_ = np.sum(self._cca.transform(X=T), axis=1)
...
class eTRCA(BaseEstimator, ClassifierMixin):
    """
    The 'ERP TRCA' [1]_, which computes ERPs as templates for full sequences and performs a TRCA for spatial filtering.

    References:
    .. [1] Nakanishi, M., Wang, Y., Chen, X., Wang, Y. T., Gao, X., & Jung, T. P. (2017). Enhancing detection of SSVEPs
...
class FilterBank(BaseEstimator, ClassifierMixin):
    """
    Filterbank wrapper around classifier object.
    """

    def __init__(self, estimator, gating="mean"):
        """
        The constructor for FilterBank.
    ...
    def fit(self, X, y):
        ...
        Args:
            X: np.ndarray
                The matrix of EEG data of shape (n_trials, n_channels, n_samples, n_passbands).
            y: np.ndarray
                The vector of ground-truth labels of the trials in X of shape (n_trials). Note, these denote the index
                at which to find the associated codes!
    ...
    def predict(self, X):
        ...
        Returns:
            np.ndarray
                The vector of predicted labels of the trials in X of shape (n_trials). Note, these denote the index at
...
class rCCA(BaseEstimator, ClassifierMixin):
    """
    The 'reconvolution CCA' method [1]_, which performs a spatial and temporal decomposition (reconvolution) within a
    CCA [2]_ to perform spatial filtering as well as template prediction [3]_.
    ...
    .. [2] Thielen, J., Marsman, P., Farquhar, J., & Desain, P. (2017). Re(con)volution: accurate response prediction
    ...
    .. [3] Thielen, J., Marsman, P., Farquhar, J., & Desain, P. (2021). From full calibration to zero training for a
    ...
    def __init__(self, codes, fs, event="duration", transient_size=0.3, onset_event=False, score_metric="correlation",
        ...
        Args:
            codes: np.ndarray
                The pseudo-random noise-codes used for stimulation of shape (n_classes, n_samples). Should be sampled at
                fs and one cycle (i.e., code-repetition) is sufficient.
            fs: int
                The sampling frequency of the EEG data in Hz.
            ...
            lx: float | list (default: None)
                ...
                If None, no regularization is applied.
            ly: float | list (default: None)
                Regularization on the covariance matrix for CCA for all or each individual parameter along M (samples).
                If None, no regularization is applied.
            latency: np.ndarray (default: None)
                The raster latencies of each of the classes of shape (n_classes,) that the data/templates need to be
                corrected for.
    ...
    def fit(self, X, y):
        ...
        if self.ensemble:
            ...
            for i_class in range(n_classes):
                T[i_class, :] = np.sum(self._cca[i_class].transform(X=None, Y=M[[i_class], :, :]), axis=1)
        else:
            T = np.sum(self._cca.transform(X=None, Y=M), axis=1)
    ...
    def set_codes(self, codes):
        ...
        Args
            codes: np.ndarray
                The pseudo-random noise-codes used for stimulation of shape (n_samples, n_codes). Should be sampled at
                fs and one code-repetition is sufficient.
        """
        self.codes = codes
        T = np.sum(self._cca.transform(X=None, Y=self._get_M(2 * self.codes.shape[1])), axis=1)
        self.Ts_ = T[:, :self.codes.shape[1]]
        self.Tw_ = T[:, self.codes.shape[1]:]
