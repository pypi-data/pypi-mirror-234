...
def correct_latency(X, y, latency, fs, axis=-1):
    """
    Correct for a latency in the EEG data by shifting data according to the class-specific latencies.
...
def correlation(A, B):
    ...
    score = np.dot(Az, Bz.T) / np.sqrt(outer)
...
def euclidean(A, B):
    ...
    score = np.zeros((A.shape[0], B.shape[0]))
    for i in range(A.shape[0]):
        for j in range(B.shape[0]):
            c = A[i, :] - B[j, :]
            score[i, j] = np.sqrt(np.inner(c, c))
    return score
...
def event_matrix(V, event, onset_event=False):
    ...
    Args:
        V: np.ndarray
            The noise-tags (i.e., pseudo-random noise-codes) used for stimulation of shape (n_codes, n_samples). Note,
            this can be any continuous time-series per code, it is not limited to binary codes.
        event: str
...
def filterbank(X, passbands, fs, tmin=None, ftype="butterworth", N=None, stopbands=None, gpass=3.0, gstop=40.0):
    """
    Applies a filterbank of a certain type to the data. Note, the order of the filter is set according to the maximum
    loss in the passband and the minimum loss in the stopband.
    ...
    Args:
        ...
        tmin: float (default: None)
            The window before trial onset that can catch any filter artefacts and will be cut off after filtering. If
            None, no data will be cut away.
        ftype: str (default: "butterworth")
            The filter type: "butterworth" | "chebyshev1"
        N: int | list (default: None)
            The filter order. If a list is provided, it is the order for each passband. If None, the order is set given
            the stopbands, gpass and gstop.
        stopbands: list(list) (default: None)
            A list of lists with a stopband for each passband defined as [lower, higher] cutoff in Hz. If None,
            the stopbands default to [lower-2, higher+7] of the passbands. Only used if N=None.
        gpass: float | list (default: 3.0)
            The maximum loss in the passband (dB). If a list is provided, it is the gpass for each passband. Only used
            if N=None.
    ...
    Returns:
        np.ndarray
            The matrix of EEG data of shape (n_trials, n_channels, n_samples, n_passbands). If tmin is not None,
            n_samples will be reduced with tmin * fs number of samples.
    """
...
def structure_matrix(E, transient_size):
    ...
    for i_event in range(n_events):
        tmp = np.zeros((n_codes, transient_size[i_event], n_samples), dtype="uint8")
        tmp[:, 0, :] = E[:, i_event, :]
    ...
    M = np.concatenate(M, axis=1).astype("uint8")
