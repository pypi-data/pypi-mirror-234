...
class eCCA(BaseEstimator, ClassifierMixin):
    """The 'ERP CCA', which is the "reference" method [1]_ which computes ERPs as templates for full sequences and
...
class eTRCA(BaseEstimator, ClassifierMixin):
    """The 'ERP TRCA' [2]_, which computes ERPs as templates for full sequences and performs a TRCA for spatial
    filtering.
...
class rCCA(BaseEstimator, ClassifierMixin):
    """The 'reconvolution CCA' method [3]_, which performs a spatial and temporal decomposition (reconvolution) within a
    ...
    def set_codes(self, codes):
        ...
        ----------
        codes: np.ndarray
            The pseudo-random noise-codes used for stimulation of shape (n_samples, n_codes). Should be sampled at fs
