...
class BakerEvent(betterproto.Message):
    ...
    baker_set_transaction_fee_commission: "BakerEventBakerSetTransactionFeeCommission" = betterproto.message_field(
        9, group="event"
    )
    ...
    baker_set_finalization_reward_commission: "BakerEventBakerSetFinalizationRewardCommission" = betterproto.message_field(
        11, group="event"
    )
...
class AccountTransactionEffects(betterproto.Message):
    ...
    encrypted_amount_transferred: "AccountTransactionEffectsEncryptedAmountTransferred" = betterproto.message_field(
        11, group="effect"
    )
...
class AuthorizationsV0(betterproto.Message):
    ...
    parameter_election_difficulty: "AccessStructure" = betterproto.message_field(4)
...
class ConsensusInfo(betterproto.Message):
    ...
    slot_duration: "Duration" = betterproto.message_field(4)
...
class BlockInfo(betterproto.Message):
    ...
    slot_number: "Slot" = betterproto.message_field(9)
...
class PoolInfoResponse(betterproto.Message):
    ...
    pool at the end of a given block.
    """
    ...
    equity_capital: "Amount" = betterproto.message_field(3)
    """The equity capital provided by the pool owner."""

    delegated_capital: "Amount" = betterproto.message_field(4)
    """The capital delegated to the pool by other accounts."""

    delegated_capital_cap: "Amount" = betterproto.message_field(5)
    """
    The maximum amount that may be delegated to the pool, accounting for
    leverage and stake limits.
    """

    pool_info: "BakerPoolInfo" = betterproto.message_field(6)
    """
...
class NodeInfoBakerConsensusInfo(betterproto.Message):
    ...
    active_baker_committee_info: "NodeInfoBakerConsensusInfoActiveBakerCommitteeInfo" = betterproto.message_field(
        3, group="status"
    )
    """
    ...
    active_finalizer_committee_info: "NodeInfoBakerConsensusInfoActiveFinalizerCommitteeInfo" = betterproto.message_field(
        4, group="status"
    )
    """
...
class SignatureMap(betterproto.Message):
    ...
    doesn't allow nested maps directly.
    """
...
@dataclass(eq=False, repr=False)
class UpdateInstructionSignature(betterproto.Message):
    signatures: Dict[int, "Signature"] = betterproto.map_field(
        1, betterproto.TYPE_UINT32, betterproto.TYPE_MESSAGE
    )
    """
    A map from `UpdateKeysIndex` to `Signature`. The type `UpdateKeysIndex`is
    not used directly, as messages cannot be keys in maps.
    """
...
class ChainParameters(betterproto.Message):
    ...
    """
    Chain parameters that apply when the block is a protocol version 4 or up
    block.
    """
