...
class CertoraBuildGenerator:
    ...
    def collect_func_source_code_signatures_source(self, contract_file: Path,
        ...
        if file_abs_path.suffix == VY:
            ...
            storage_data = smart_contract_lang.collect_storage_layout_info(str(file_abs_path), compilation_path,
                                                                           solc, None,
                                                                           standard_json_data)
            abi = storage_data[CONTRACTS][str(contract_file)][contract_name]["abi"]
            ast_logger.debug(f"abi is: \n{abi}")
            for f in filter(lambda x: self.is_imported_abi_entry(x), abi):
                func_signature = self.get_full_func_signature(f)
                ast_logger.debug(f"Collected function signature {func_signature} from ABI")
    ...
    def standard_json(self,
        ...
        if compiler_collector_lang == CompilerLangSol():
            sources_dict = {str(contract_file_as_provided): {
    ...
    def collect_for_file(self,
                         build_arg_contract_file: str,
                         contract_qualifier_name_like: str,
                         file_index: int,
        ...
        @param contract_qualifier_name_like - the file name as solidity expects (yikes solidity!)
        ...
        # Standard JSON
        input_for_solc = self.standard_json(Path(file_abs_path), contract_qualifier_name_like, remappings,
                                            smart_contract_lang)
        ...
        while contracts_to_add_dependencies_queue:
            ...
            elif contract_file == file_compiler_path:
                # try the qualifier instead
                contracts_to_add_dependencies_queue.append(Path(contract_qualifier_name_like))
                continue
            else:
        ...
        build_logger.debug(
            ...
            f"{contracts_with_libraries.get(contract_qualifier_name_like, None)}")
        contracts_with_chosen_addresses = \
        ...
        report_source_file = report_srclist[[idx for idx in srclist if srclist[idx] == contract_qualifier_name_like][0]]
        ...
        for contract_file, contract_list in sorted(list(contracts_with_libraries.items())):
            for contract_name in contract_list:
                source_file = Util.abs_posix_path(contract_file)
        ...
        for primary_contract in contracts_in_file:
            ...
            primary_contract_address = \
                self.find_contract_address_str(contract_qualifier_name_like,
                                               primary_contract,
                                               contracts_with_chosen_addresses)
    ...
    def get_contract_in_sdc(self,
                            contract_qualified_filename: str,
                            contract_name: str,
                            contracts_with_chosen_addresses: List[Tuple[int, Any]],
                            data: Dict[str, Any],
                            ...
                            contract_file_abspath: str
                            ) -> ContractInSDC:
        contract_data = data[CONTRACTS][contract_qualified_filename][contract_name]
        ast_logger.debug(f"Contract {contract_name} is in file {contract_qualified_filename}")
        compiler_lang = compiler_collector_for_contract_file.smart_contract_lang
        if compiler_lang == CompilerLangSol():
            lang = "Solidity"
            types = self.collect_source_type_descriptions(contract_qualified_filename, contract_name, build_arg_contract_file,
                                                          compiler_lang)
            funcs = self.collect_funcs(contract_data, contract_qualified_filename, contract_name, build_arg_contract_file, types)
        else:
            lang = compiler_lang.name
            (types, clfuncs) = compiler_lang.collect_source_type_descriptions_and_funcs(self.asts, contract_data,
                                                                                        contract_qualified_filename, contract_name,
                                                                                        build_arg_contract_file)
            funcs = [Func(
                name=x.name,
                fullArgs=x.fullArgs,
        ...
        source_size = self.collect_contract_bytes(contract_qualified_filename, contract_name, build_arg_contract_file)
        ...
        address = self.find_contract_address_str(contract_qualified_filename,
                                                 contract_name,
                                                 contracts_with_chosen_addresses)
        storage_layout = \
    ...
    def build(self) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            sdc_pre_finders = self.collect_for_file(build_arg_contract_file, build_arg_contract_file, i, compiler_lang,
                                                    Path(os.getcwd()))
    ...
    def instrument_auto_finders(self, build_arg_contract_file: str, i: int,
        ...
        try:
            ...
            # set path for cwd for solc building autofinders. usually cwd will also be the base of sources dir,
            # but if not, we need to append the relative path from our current location to the base
            path_to_build_autofinders = Util.get_certora_sources_dir() / self.cwd_rel_in_sources
            new_sdcs = self.collect_for_file(new_file, build_arg_contract_file, i,
                                             get_compiler_lang(build_arg_contract_file),
                                             path_to_build_autofinders,
