...
def collect(args: argparse.Namespace) -> bool:
    ...
    if original_results is not None:
        ...
        orig_collect_success = False
        print(f"Failed to get results for original. This means the report may not get generated correctly.")

    # check the mutant URLs
    mutants_objs = results_work[Constants.MUTANTS]
    mutant_collect_success = True
    if any([(Constants.LINK not in mutant) or (not valid_link(mutant[Constants.LINK])) or (mutant[Constants.LINK] is None)
