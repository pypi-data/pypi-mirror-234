...
# Get the path to the JSON file within the package
package_dir = os.path.dirname(__file__)
json_file_path = os.path.join(package_dir, 'accipy_data', 'possible_values.json')
...
def query(*args, download = False, require_all = True):
    ...
    if download == False:
        ...
        pool = Pool(initializer=init, initargs=(ql, fl, cl), processes=num_processes)
        ...
        # Aggregate partial csv files into one
...
if __name__ == '__main__':
    ...
    query(("Factual narrative", "contains", "fire"))        
