    ...
    def list(params=None, filter_fields=None, only_first=False, **kwargs):
        ...
        # 分页返回
            ...
            for j in page_list:
                ...
                if "invite" in filter_params.get("relate_key", []):
                    #  判断该用户的邀请人是否是业务人员
                    role_info, err = RoleService.get_role_list(params={"user_id": j["with_user_id"]}, only_first=True)
                    is_salesperson = False
                    j['sale_username'] = ""
                    if role_info and role_info.get("role_key") == "BID-SALESMAN":
                        is_salesperson = True
                    if is_salesperson:  # 是业务人员直接取with_user的数据
                        j['sale_username'] = j["with_real_name"] if j["with_real_name"] else j["with_nickname"]
                    else:  # 不是业务员取最终受益人是否是业务员
                        ancestor = UserRelateToUser.objects.filter(user__is_delete=0, with_user__is_delete=0,
                                                                   user_relate_type_id=2,
                                                                   user_id=j["user_id"]).first()
                        if ancestor:
                            data, err = DetailInfoService.get_detail(ancestor.with_user.id, filter_fields="real_name")
                            if data:
                                j['sale_username'] = data["real_name"] if data["real_name"] else ancestor.with_user.user_name
