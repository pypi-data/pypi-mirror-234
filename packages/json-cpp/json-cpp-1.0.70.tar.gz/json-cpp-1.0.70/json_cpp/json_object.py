...
class JsonObject:
    ...
    def __dataframe_values__(self):
        return [v.to_dataframe(recursive=True) if isinstance(v, (JsonList, JsonObject)) else v for v in self.get_values()]
    ...
    def to_dataframe(self, recursive: bool = False):
        ...
        if recursive:
            ...
            return DataFrame([self.get_values()], columns=columns)
...
class JsonList(list):
    ...
    def to_dataframe(self, recursive: bool = False):
        ...
        if issubclass(self.list_type, JsonObject):
            if self.list_type is JsonObject:
                if len(self) == 0:
                    return DataFrame()
