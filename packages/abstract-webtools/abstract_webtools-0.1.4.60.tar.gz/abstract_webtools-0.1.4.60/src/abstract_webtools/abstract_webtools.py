...
class UrlManager:
    ...
    Methods:
        ...
        get_domain_name(url): Get the domain name from a URL.
    ...
    def url_join(self,url,path):
        protocol,domain,path,query = url_to_pieces(url=url)
        url = eatOuter(url,['/'])
        path = eatInner(path,['/'])
        url = url+'/'+path
        return url
...
class SafeRequest:
    ...
    def authenticate(self,s, login_url=None, email=None, password=None,checkbox=None,dropdown=None):
        login_urls = login_url or [self.url_manager.url,self.url_manager.domain_name,self.url_manager.url_join(url=self.url_manager.domain_name,path='login'),self.url_manager.url_join(url=self.url_manager.domain_name,path='auth')]
    ...
    def initialize_session(self):
        ...
        if self.auth != None:
            s.auth = self.auth
        elif self.password or self.email or self.login_url or self.checkbox or self.dropdown:
            s=self.authenticate(s, login_url=self.login_url, email=self.email, password=self.password, checkbox=self.checkbox, dropdown=self.dropdown)
