...
class SafeRequest:
        def __init__(self, url, _url_manager, soup_manager=None, headers=None, last_request_time=None, max_retries=3, request_wait_limit=1.5):
            self.url = url
            self._url_manager = _url_manager.get_instance(url=self.url)  # fixed this line
            self.request_manager = SafeRequestSingleton.get_instance(self.url_mananger.url)
        ...
        def try_request(self,timeout=10) -> (requests.Response or None):
            ...
            'SafeRequest' object has no attribute '_url_manager'
            Args:
...
class SoupManager:
    def __init__(self, url_manager,request_manager, parse_type="html.parser"):
        ...
        self.soup= BeautifulSoup(self.request_manager.source_code, parse_type)
        self.meta_tags =  {}
        self.get_meta_tags()
        self.all_desired = []
        ...
        self.all_urls = []
        self.all_links = None  # This is a "private" attribute where the actual data is stored

    @property
    def _all_links(self):
        """This is a property that provides access to the _data attribute.
        The first time it's accessed, it will load the data."""
        if self._data is None:
            print("Loading all links for the first time...")
            self.all_links = self.get_all_website_links()
    ...
    def get_react_source_code(self) -> list:
            ...
            script_tags =  self.soup.find_all('script', type=lambda t: t and ('javascript' in t or 'jsx' in t))
