...
class Default:
  ...
  class Space:
    ...
    def list_planes(
      ...
      parsed = None
      if response.status == 200:
        parsed = models.parse_(json.loads(response.data))
...
class Vision:
  ...
  class Cameras:
    ...
    def list_cameras(
      ...
      Union[
        models.PaginatedCamera,
        None
      ],
      models.PaginatedCamera
    ]:
      """
      List cameras connected to the robot
      """
      path = "/api/v1/cameras"
      response = self._request_manager.request(
        "get",
      ...
      parsed = None
      if response.status == 200:
        parsed = models.parse_paginated_camera(json.loads(response.data))
    ...
    def get_camera_frame_rgb(
      ...
      """
      Get the current RGB image from the camera
      """
      path = "/api/v1/cameras/{cameraId}/frame/rgb"
      path = path.replace("{cameraId}", str(camera_id))
      response = self._request_manager.request(
        "get",
        path,
        headers=self._request_manager.json_headers(),
  ...
  def __init__(self, request_manager: RequestManager):
    self._request_manager = request_manager
    self.cameras = Vision.Cameras(request_manager)
...
class StandardBotsRobot(Default):
  ...
  def __init__(
    ...
    self.vision = Vision(self._request_manager)
