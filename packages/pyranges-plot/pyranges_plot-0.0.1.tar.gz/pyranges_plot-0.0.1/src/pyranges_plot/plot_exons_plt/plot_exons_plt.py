...
def _gby_plot_exons(df, axes, fig, chrmd_df, genesmd_df):
    """
    Takes the DataFrame object corresponding to the data of 1 gene in a
    pyranges object and plots it in its corresponding chromosome.
    
    Parameters:
        df: DataFrame with the gene's data
        chrs: number of chromosomes in the plot
        axes: axes plot object
        fig: figure object
...
def _apply_gene(row, fig, ax, strand, genename, gene_ix, exon_color, chrom, chrom_ix, n_exons):
    ...
    """Takes grouped rows of a DataFrame belonging to a gene's exons and plots them."""
