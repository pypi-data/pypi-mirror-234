...
def terminal_interface(interpreter, message):
    if not interpreter.auto_run:
        display_markdown_message("""**my-interpreter** 在运行代码之前需要您确认，您可使用“my-interpreter -y”命令，执行代码时就不再询问是否执行代码。
    ...
    while True:
        ...
        if message.strip() == "interpreter --local":
            print("请按 CTRL-C 然后运行 ​​`my-interpreter --local`。")
            continue
        ...
        try:
            for chunk in interpreter.chat(message, display=False, stream=True):
                if interpreter.debug_mode:
                    print("`terminal_interface` 中的块:", chunk)
