...
class Olive:
    def __init__(self, namespace, alias='worker'):
    ...
    def run(self):
        self.logger = Log(self.home, self.alias).create_logger()
        self.logger.info("------------------------------------------------")
        self.logger.info("main, run %s", self.namespace)

        self.adapter = Adapter(self.home, self.logger)
        if not self.adapter.open():
            return

        self._create_pid_file()
        # channel manager start
        self.channel = Channel(self.home, self.logger)
        self.channel.start()
