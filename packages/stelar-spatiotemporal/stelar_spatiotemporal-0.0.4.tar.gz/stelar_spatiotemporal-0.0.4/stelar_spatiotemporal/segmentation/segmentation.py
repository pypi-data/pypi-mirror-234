...
def combine_rgb_npys_into_eopatch(bands_data_package:BandsDataPackage, outdir:str, dates:list = None) -> str:
    if dates is None:
        dates = [dt.datetime.strptime(os.path.basename(pair[0]).replace(".RAS",""), "%Y_%m_%d") for pair in bands_data_package.B2_package.path_pairs]
    ...
    for band_data_package in bands:
        for segment_date in dates:
            if not os.path.exists(os.path.join(band_data_package.BAND_PATH, segment_date.strftime("%Y_%m_%d.npy"))):
    ...
    for band_data_package in bands:
        ...
        npy_paths = [os.path.join(band_data_package.BAND_PATH, date.strftime("%Y_%m_%d.npy")) for date in dates]
