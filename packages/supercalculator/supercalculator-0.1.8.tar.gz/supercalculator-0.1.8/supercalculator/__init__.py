...
def exponent(value : str, exp : str) -> str:
    ...
    if (value.split(".")[0] == "1" + "0"*(len(value.split(".")[0])-1)) and (not "." in value or (value.split(".")[-1] == "0" * len(value.split(".")[-1]))):
        for x in range(0, int(multiplication(str(val), str(len(value.split('.')[0][1:]))).split(".")[0])):
            print(f"{x}/{int(multiplication(str(val), str(len(value.split('.')[0][1:]))).split('.')[0])}")
            final_value = f"{final_value}0"
    else:
...
def square_root(value : str, precision : int = None) -> str:
    ...
    while True:
        i += 1
        if l > precision or (int(reg3) == 0 and 2*i - offset>= len(value)):
            break
...
def _round(value : str, decimals : int = 0) -> str:
    ...
    if decimals >=0 and len(value_dec) >= decimals and int(value_dec[decimals]) >= 5:
...
def __calc_lim(parsed_equation : dict, x_goes_towards : str, side, precision : int = None) -> str:
    ...
    if lim == _division_by_0_error: return lim
...
def __parser(expression : str):
    ...
    for char in expression:
        ...
        if char == "(":
            ...
            for op in operators:
                if op in read_queue:
                    op_index = read_queue.index(op) + 1
                    break

            read_queue = read_queue[op_index:]
            if read_queue in functions:
                ...
                parentheses += 1
                if current_lim != None and read_queue != 'lim(':
    ...
    #     parsed.update({x[0] : [i for i in x[-1] if i != None]})
