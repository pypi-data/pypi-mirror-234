...
class SyncStep(JsonList):
    ...
    def __getitem__(self, index):
        if isinstance(index, str):
            return self.find_first(lambda s:s.agent_name == index, not_found_behavior=NotFoundBehavior.ReturnNone)
...
class SyncTrajectories(JsonObject):
    def __init__(self, trajectories: Trajectories):
        self.syncSteps = JsonList(list_type=SyncStep)
        split_trajectories = trajectories.split_by_agent()
        self.agent_names = JsonList(list_type=str, iterable=split_trajectories)
        min_frame = trajectories[0].frame
        max_frame = trajectories[-1].frame
        split_trajectories = trajectories.split_by_agent()
