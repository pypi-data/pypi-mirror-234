...
# Maps HTML attributes that are invalid Python identifiers to their Dotreact
# prop equivalents.
...
def attr_to_prop(attr_name: str) -> str:
    """Convert an HTML attribute name to its Dotreact name.
    ...
    name to a Dotreact name.

    Args:
        attr_name: The HTML attribute name.

    Returns:
        A Dotreact prop name that maps to the HTML attribute.
    """
...
# Names of HTML attributes that are provided by Dotreact out of the box.
DOTREACT_PROVIDED_ATTRS = {"class", "id", "style"}

# ATTR_TO_ELEMENTS contains HTML attribute names, which might be invalid as
# Dotreact prop names. PROP_TO_ELEMENTS contains the corresponding Dotreact
# prop names. It omits props that are provided by Dotreact out of the box.
PROP_TO_ELEMENTS = {
    attr_to_prop(attr_name): elements
    for attr_name, elements in ATTR_TO_ELEMENTS.items()
    if attr_name not in DOTREACT_PROVIDED_ATTRS
}
