...
def process_embedding(sl, rv):
    import pickle
    # embedding,{dt.timestamp()},{cw_url},{eann},{emsg},{eem}
    print(f"process_embedding: Entered. len(sl)={len(sl)}", flush=True)
    dann = base64.b64decode(sl[3].strip()).decode('ascii') # annotation
    dmsg = base64.b64decode(sl[4].strip()).decode('ascii') # message
    demb = pickle.loads(base64.b64decode(sl[5].strip())) # embedding
    rv.append((sl[1], dann, dmsg, sl[2], demb))
...
def load_from_db(fn):
    ...
    with open(fnu, 'r') as f_in:
        for line in f_in:
            ...
            print(f"load_from_db: len rv {len(rv)}, len rv1 {len(rv1)}", flush=True)
    return rv, rv1
