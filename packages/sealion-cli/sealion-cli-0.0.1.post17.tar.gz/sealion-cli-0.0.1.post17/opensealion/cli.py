...
def show_introduction(stdscr, introduction: str) -> int:
    ...
    start_row += 1
    _x = max(width//2 - len(version)//2, 0)
    stdscr.refresh()  # 刷新屏幕
    stdscr.addstr(start_row, _x, 'v-' + version)
...
def do_sealion_boot(stdscr):
    ...
    y += 1
    version, esc_flag = get_inputs_and_echo(stdscr, y, x)
    y += 1
    ...
    while not break_flag:
        ...
        if sub_key == curses.KEY_UP:
            ...
            if sub_current_row == len(sub_items) - 1:
                ...
                # install create-mm-app
                # elif _row == install_items[3]:
                #     do_install_mm_cli(stdscr)
                elif _row == themes[0]:
                    COLOR_TYPE = 1
                elif _row == themes[1]:
                    COLOR_TYPE = 2
