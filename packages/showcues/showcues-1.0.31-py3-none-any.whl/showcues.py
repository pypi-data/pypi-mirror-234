...
class Scte35Profile:
    ...
    def __init__(self):
        ...
        A Scte35Profile
        """
        show_cues = False  # Show SCTE-35 Cues fully expanded.
        ...
        self.descriptor_tags = [ 2,]# Which Descriptors to parse.
    ...
    def format4profile(self, this, that):
        if this is None or that is None:
            return
        this = this.lower()
        if this.startswith("parse"):
            if that[0].lower().startswith("f"):
                that = False
            else:
                that = True
        if this in ["command_types", "descriptor_tags"]:
            that = [int(tt) for tt in that]

        if this in ["starts", "stops"]:
            that = [int(s, 16) for s in that]
...
class CuePuller:
    def __init__(self):
        ...
        with open(self.sidecar, "w+") as sidecar:  # touch
            pass
        with open(self.dumpfile, "w+") as dump:
            pass
    ...
    def diff_stuff(self):
        ...
        if self.break_timer is not None:
            ...
            return f"{NSUB}Timer: { round(self.break_timer,6)}{NSUB}Duration: {self.break_duration}{NSUB}Diff: {round(self.break_timer - self.break_duration,6)}"
        return ""
    ...
    def set_cue_state(self, cue, line):
        ...
        if "CONT" not in line:
            ...
            if line.startswith("#EXT-X-CUE-IN") and self.cue_state == "CONT":
                ...
                self.reset_break()
                return line
            if line.startswith("#EXT-X-CUE-OUT") and self.cue_state == None:
                self.cue_state = "OUT"
...
[ SCTE-35 Parsing Profiles ]
    ...
    hls_tags = #EXT-OATCLS-SCTE35,#EXT-X-CUE-OUT-CONT,#EXT-X-DATERANGE,#EXT-X-SCTE35,#EXT-X-CUE-IN,#EXT-X-CUE-OUT
    command_types = 6,5
    descriptor_tags = 2
    starts = 34,48,50,52,54,68,70
    stops = 35,49,51,53,55,69,71
    ...
    1) Values do not need to be quoted.
    2) Multiple values are separated by a commas.
    3) No partial line comments. Comments must be on a separate lines.
    4) Comments can be started with a # or //
...
[ Output Files ]
    ...
    sc.m3u8  - live playable rewrite of the m3u8
               with the SCTE-35 profile rules applied. 
