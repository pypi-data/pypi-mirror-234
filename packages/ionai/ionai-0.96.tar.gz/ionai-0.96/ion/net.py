...
RESPONSE_SUCCESS = __RESPONSE_SUCCESS
...
class server_cls():
		...
		try:
			...
			while True:
				try:
					...
					elif (self._data_processing(cli_sock, cli_message)):
	...
	def query_processing(self, cli_sock, cli_query):  # Function for Override
		if (cli_query == 'query #1' or cli_query == 'query #2' or cli_query == 'query #3'):
			svr_response = _RESPONSE_SUCCESS
			cli_sock.sendall(_encode(svr_response))
			return True
		else:
			svr_response = _RESPONSE_ERROR
			cli_sock.sendall(_encode(svr_response))
			return False
