...
from src.entities.invoice import Invoice, InvoiceOkvendas
                    ...
                    for invoice in invoices:
                        try:
                            ...
                            if invoice_sent is None:
                                ...
                                send_log(job_config.get('job_name'), job_config.get('enviar_logs'), True,
                                         f'Falha ao enviar NF do pedido {invoice.pedido_oking_id} para api okvendas: {invoice_sent}',
    ...
    try:
        ...
        if db_config.is_sql_server():
            cursor.execute('SET NOCOUNT ON;'
                           ...
                           'exec openk_semaforo.sp_atualiza_pedido @pedido = ?, @status = ?, @pedido_erp = @pedido_erp_output',
                           'SELECT @pedido_erp_output as pedido_erp',
                           (pedido_oking_id, status))
            row = cursor.fetchone()
            if row[0] > 0:
                updated = True
    ...
    for i in sales:
        ...
        if i["codigo_cliente"] not in aux:
            ...
            dicio["data_validade"] = (datetime.strptime(str(i["data_validade"]).replace('T', ' '),
                                                        '%Y-%m-%d %H:%M:%S.%f').strftime("%Y-%m-%d %H:%M:%S")) if i["data_validade"] is not None else None
            dicio["items"] = []
