...
from .agent import Player, FixChameleonPlayer, ChameleonPlayer, FixUndercoverPlayer, PGMPlayer
from .environments import Environment, TimeStep, load_environment
from .backends import Human
from .config import ArenaConfig
from ..prompts.chameleon_pgm import chameleon_global_prompt_pgm
from ..prompts.under_cover_pgm import undercover_clue_demo, undercover_view_demo, global_prompt_undercover_pgm
...
class Arena:
    ...
    def step(self, request_msg=None) -> TimeStep:
        ...
        for i in range(self.invalid_actions_retry):  # try to take an action for a few times
            if isinstance(player, FixChameleonPlayer):
                action = player(observation, request_msg=request_msg, stage=self.environment._current_phase)
            elif isinstance(player, ChameleonPlayer):
                action = player(observation, request_msg=request_msg, stage=self.environment._current_phase)  # take an action
            elif isinstance(player, FixUndercoverPlayer):
                action = player(observation, request_msg=request_msg, stage=self.environment._current_phase)  # take an action
            else:
                action = player(observation, request_msg=request_msg, single_history=self.environment._single_history)
    ...
    def run(self, num_steps: int = 1):
        ...
        for i in range(num_steps):
            ...
            # print("in run, request msg,  ",  request_msg)
