...
from xhs_spider.pojo.note import Note_Detail
from xhs_spider.pojo.user import User_Detail

current_directory = os.path.dirname(os.path.abspath(__file__))
file_path = os.path.abspath(os.path.join(current_directory, '../static', 'info.js'))
js = execjs.compile(open(file_path, 'r', encoding='utf-8').read())
...
def check_cookies():
    ...
    params = get_params()
    headers = get_headers()
    print(f'github: https://github.com/cv-cat/Spider_XHS')
    try:
        if not os.path.exists("./cookies.txt"):
            with open("./cookies.txt", "w", encoding="utf-8") as f:
                file_path = os.path.abspath(os.path.join(current_directory, '../static', 'cookies.txt'))
                cookies_template = open(file_path, 'r', encoding='utf-8').read()
                f.write(cookies_template)
            raise Exception("初次运行, 生成cookies.txt, 请手动更改cookies.txt文件")
        ...
        try:
            if not os.path.exists("./node_modules"):
                cmd_string = 'npm i jsdom'
                info = subprocess.Popen(cmd_string, shell=True, stdout=None, stderr=None).wait()
                if info == 0:
                    print('安装jsdom成功')
                else:
                    print('安装jsdom失败')
                    raise Exception("安装jsdom失败")
            ret = js.call('get_xs', api, '', a1)
        except Exception as e:
            print(f'缺少nodejs环境, 请先安装nodejs, 再运行npm i jsdom, {e}')
            return
