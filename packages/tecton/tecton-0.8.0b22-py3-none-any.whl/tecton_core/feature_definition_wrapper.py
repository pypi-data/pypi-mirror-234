...
class TimeWindow:
    ...
    @classmethod
    def from_proto_time_window(cls, time_window: feature_view__data_pb2.TimeWindow) -> "TimeWindow":
        return cls(time_window.window_start.ToTimedelta(), time_window.window_end.ToTimedelta())


def aggregation_groups(
    fdw: FeatureDefinitionWrapper, exclusive_end_time: datetime.datetime
) -> Tuple[AggregationGroup, ...]:
    aggregation_map = defaultdict(list)
    for aggregation in fdw.trailing_time_window_aggregation.features:
        aggregation_map[TimeWindow.from_proto_time_window(aggregation.time_window)].append(aggregation)

    # TODO(compaction): likely this should be an ordering driven by the backend
    sorted_windows = sorted(aggregation_map.keys())

    return tuple(
        AggregationGroup(
            name=f"agg_{i}",
            inclusive_start_time=exclusive_end_time + time_window.window_start,
            exclusive_end_time=exclusive_end_time + time_window.window_end,
            aggregate_features=tuple(aggregation_map[time_window]),
        )
        for i, time_window in enumerate(sorted_windows)
    )
