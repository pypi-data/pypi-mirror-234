    ...
    {%- if include_feature_view_timestamp_columns -%}
        {%- for item in feature_set_items -%}
            {%- if item.aggregation -%}
                ...
                DATEADD('SECOND', -MOD(DATE_PART(EPOCH_SECOND, {{ spine_table_name }}.{{ spine_timestamp_key }}), {{ item.aggregation.slide_interval.ToSeconds() }}), DATE_TRUNC('SECOND', {{ spine_table_name }}.{{ spine_timestamp_key }})) AS {{ item.name|upper + separator + item.timestamp_key|upper }},
            {%- else %}
    ...
    {%- for item in feature_set_items -%}
        ...
        {%- if item.aggregation -%}
            {%- for feature in item.aggregation.features %}
                {{- macros.final_select_name(item.namespace + separator + feature.output_feature_name, feature.function|snowflake_function == "count", not (loop.last and last_item), ("" if append_prefix else feature.output_feature_name)) | indent }}
            {%- endfor -%}
        {%- else %}
            {%- for feature in item.features %}
                {{- macros.final_select_name(item.namespace + separator + feature, false, not (loop.last and last_item), ("" if append_prefix else feature)) | indent }}
            {%- endfor -%}
        {%- endif -%}
    {%- endfor %}
