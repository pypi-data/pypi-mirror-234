...
def check_spark_version(config: ClusterConfig):
    if config.HasField("new_databricks") and config.new_databricks.HasField("spark_version"):
        expected_spark_version = config.new_databricks.spark_version
        # eg '9.1'
        actual_spark_version = os.environ.get("DATABRICKS_RUNTIME_VERSION")
    elif config.HasField("new_emr") and config.new_emr.HasField("spark_version"):
        expected_spark_version = config.new_emr.spark_version
        try:
