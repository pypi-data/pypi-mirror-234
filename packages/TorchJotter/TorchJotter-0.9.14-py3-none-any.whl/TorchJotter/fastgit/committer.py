...
class Committer:
    ...
    def _find_config_file(self, run_file_path: str = None, cli: bool = True) -> str:
        ...
        while 1:
            ...
            if os.path.isfile(os.path.join(path, config_file_name)):
                ...
                self.work_dir = path
                return path
            if os.path.isfile(os.path.join(path, ".TorchJotter", config_file_name)):
                self.config_file_path = os.path.join(path, ".TorchJotter", config_file_name)
                self.work_dir = path
                return path
    ...
    def _get_path_names(work_dir: str):
        TorchJotter_path = os.path.join(work_dir, ".TorchJotter")
        ...
        return TorchJotter_path, git_path, git_backup_path, gitignore_path, gitignore_backup_path
    ...
    def _switch_to_fast_git(work_dir: str):
        ...
        """
        TorchJotter_path, git_path, git_backup_path, gitignore_path, gitignore_backup_path = Committer._get_path_names(
            work_dir)
        ...
        if os.path.exists(TorchJotter_path):
            shutil.move(TorchJotter_path, git_path)
        if os.path.isfile(os.path.join(git_path, ".gitignore")):
            # 使用 .TorchJotter 下的 .gitignore
    ...
    def _switch_to_standard_git(work_dir: str):
        ...
        if os.path.exists(git_path):
            shutil.move(git_path, TorchJotter_path)
        if os.path.exists(gitignore_path):
            # 把 .gitignore 移动到 .TorchJotter 下
            shutil.move(gitignore_path, os.path.join(TorchJotter_path, ""))
    ...
    def _check_directory(work_dir: str, fix: bool = False, wait: bool = False) -> bool:
        """检查指定目录是否已经存在 TorchJotter 项目，同时会修复可能错误的存在

        :param work_dir: 工作目录的绝对路径
        :param fix: 是否进行自动修复，用于 TorchJotter init 模式
        :param wait: 是否等待其它使用 TorchJotter 的程序，用于 commit 前的检查
        :return: 返回是否存在 TorchJotter 项目
        """
        TorchJotter_path, git_path, git_backup_path, gitignore_path, gitignore_backup_path = Committer._get_path_names(
            work_dir)
        
        if os.path.exists(TorchJotter_path) or os.path.exists(git_backup_path) or os.path.exists(git_path):
            #  存在 .TorchJotter 或者存在 .git 变成的 .git_backup
            if fix:
                if os.path.exists(TorchJotter_path) and os.path.exists(git_backup_path):
                    # 把 .git_backup 恢复成 .git
                    shutil.move(git_backup_path, git_path)
                elif os.path.exists(git_path) and os.path.exists(git_backup_path):
                    # 先把 .git 变成 .TorchJotter ， 再把 .git_backup 恢复成 .git
                    shutil.move(git_path, TorchJotter_path)
                    shutil.move(git_backup_path, git_path)
                if os.path.isfile(gitignore_backup_path):
                    # 如果存在 .gitignore_backup , 就恢复成 .gitignore
                    shutil.move(gitignore_backup_path, gitignore_path)
                print(_colored_string("TorchJotter project has been initialized. ", "blue"))
            elif wait:
                WAITING_TIME = 30
                WAITING_ROUND = 10
                for i in range(WAITING_ROUND):
                    ...
                    cnt = WAITING_TIME
                    while cnt > 0:
                        # 如果不存在 .TorchJotter_path 或 存在 .git_backup 则等待 30s（可能是其它程序在 commit）
                        if not os.path.exists(git_backup_path) and os.path.exists(TorchJotter_path):
                            return True
                        time.sleep(1)
                        cnt -= 1
                    ...
                    print(
                        _colored_string(
                            "It seems like other TorchJotter is trying to auto-commit too "
                            "(If there is no other TorchJotter running, this is a bug)"
            ...
            # 如果都不存在，则认为 TorchJotter 项目文件夹损坏
            if wait:
                print(_colored_string(".TorchJotter folder is not found", "red"))
            return False
    
    def _commit(self, commit_message: str) -> None:
        """使用 TorchJotter 进行自动 commit, 只加入符合规则的文件
        
        :param commit_message: TorchJotter 自动 commit 的 log
        """
        ...
        if flag:
            ...
            raise RuntimeError("TorchJotter's .gitignore is damaged or mixed with your own .gitignore.")
    ...
    def _save_log(self, logs: List[str]):
        """ 将要存储的信息存储到默认的 TorchJotter

        :param logs: 要存储的信息
        :return:
        """
        with open(os.path.join(self.work_dir, ".TorchJotter", "fit_logs"), "a", encoding='utf8') as file_out:
    ...
    def _get_commits(self, cli: bool = False) -> Info:
        ...
        try:
            master = os.path.join(work_dir, *"/.TorchJotter/logs/refs/heads/master".split('/'))
    ...
    def _revert(self, commit_id: str, path: str = None, cli: bool = False, id_suffix: bool = False) -> Info:
        ...
        if self._check_directory(work_dir):
            ...
            if not flag:
                ...
                core_path = os.path.join(path, ".TorchJotter")
                if os.path.exists(path):
                    shutil.rmtree(path, ignore_errors=True)
                os.makedirs(path)
                shutil.rmtree(core_path, ignore_errors=True)
                shutil.copytree(os.path.join(work_dir, ".TorchJotter"), core_path)
            ...
            if cli:
                print(_colored_string('Not in a TorchJotter directory', 'red'))
            return Info(1, "Error: Not in a TorchJotter directory")
    ...
    def commit(self, file: str, commit_message: str = None) -> Info:
        ...
        if commit_message is None:
            commit_message = "Commit by TorchJotter"
        ...
        if not flag:
            return Info(1, "Error: TorchJotter project is damaged")
        logs = [_arguments_flag, "Run ", " ".join(sys.argv), "\n"]
        logs += [_system_flag]
        self._switch_to_fast_git(self.work_dir)
        try:
            self._commit(commit_message)
            print(_colored_string('Auto commit by TorchJotter', 'blue'))
    ...
    def TorchJotter_last_commit(self) -> Commit:
    ...
    def TorchJotter_commits(self) -> List[Commit]:
    ...
    def fit_last_commit_info(work_dir: str) -> Info:
        """获取 work_dir 或其祖其先目录上 TorchJotter 的上一次 commit 信息
    ...
    def init_project(self, pj_name: str, version: str = "normal", hide: bool = False, git: bool = True) -> int:
        """命令行用来初始化一个 TorchJotter 项目

        :param pj_name: 项目名称
        :param version: 项目初始化文件夹的版本，目前只有 normal 这一种
        :param hide: 是否隐藏 .fitconfig 文件到 .TorchJotter 文件夹中
        :param git: 是否在初始化 TorchJotter 的同时为项目初始化 git
        ...
        print(_colored_string("TorchJotter project %s is initialized." % pj_name, "green"))
        return 0
    ...
    def short_logs(self, show_now: bool = False, last_num: int = None):
        ...
        if self._check_directory(os.path.abspath('.')):
            try:
                ...
                with open(os.path.join('.TorchJotter', 'fit_logs'), 'r', encoding='utf8') as fin:
                    lines = fin.readlines()
                cnt = 0
                show_logs = []
                for line in lines:
                ...
                print("No TorchJotter records here.")
        else:
            print(_colored_string('Not in a TorchJotter directory', 'red'))
    
    def TorchJotter_revert(self, commit_id: str, run_file_path: str = None, id_suffix: bool = False) -> Info:
        """TorchJotter 调用此接口进行版本回退
