...
def _get_config_args(conf: TorchJotterConfig):
    """
    读取TorchJotterConfig内的超参。
    """
...
def _check_log_dir(func):
    ...
    def wrapper(*args, **kwargs):
        if not args[0].initialized and args[0].default_log_dir is not None:
            ...
            raise RuntimeError("You have to call `TorchJotter.set_log_dir()` to set where to save log first.")
        return func(*args, **kwargs)
...
class Logger:
    ...
    def commit(self, file: str=None, fit_msg: str = None):
        ...
        :param file: 以该路径往上寻找.TorchJotter所在文件夹。一般传入__file__即可, 如果为 None，则通过调用栈自动获取
        ...
        if msg['status'] == 0:  # 成功了
            ...
            print(
                _colored_string("Put your code inside a TorchJotter project, or use 'TorchJotter init' to initialize it\n", "red")
            )
            raise RuntimeError(
                "It seems like you are not running under a folder governed by TorchJotter.\n" + msg['msg']
            )
    ...
    def _create_log_files(self):
        ...
        if not hasattr(self, 'meta_logger'):
            ...
            for name in ['meta', 'hyper', 'metric', 'other', 'loss', 'progress', 'best_metric', 'file']:
                logger_name = 'TorchJotter_{}'.format(name)
    ...
    def finish(self, status: int = 0, send_to_bot: str = None):
        ...
        if send_to_bot is not None:
            if isinstance(send_to_bot, str):
                if status == 0:
                    title = "[ TorchJotter 训练完成 ]"
                    text = "TorchJotter 提醒您：您的训练任务已完成！"
                else:
                    title = "[ TorchJotter 训练错误 ]"
                    text = "TorchJotter 提醒您：您的训练任务发生了错误。"
                data = {
                    "msg_type": "post",
                    "content": {
                        "post": {
    ...
    def add_hyper(self, value: Union[int, str, float, dict, argparse.Namespace, ConfigParser], name=None):
        ...
        elif inspect.isclass(value) and issubclass(value, TorchJotterConfig):
            value = _get_config_args(value)
        elif isinstance(value, TorchJotterConfig):
            value = _get_config_args(value)
        else:
            try:
                import dataclasses
    ...
    def add_other(self, value: Union[int, str, float, dict], name: str = None):
        ...
        if name in ('meta', 'hyper', 'metric', 'loss') and not isinstance(value, dict):
            raise KeyError("Don't use {} as a name. Use TorchJotter.add_{}() to save it.".format(name, name))
    ...
    def add_hyper_in_file(self, file_path: str=None):
        ...
        如果多次出现，只会记录第一次出现的值。另外等号最右侧的不能是一个变量，TorchJotter无法知道变量取什么值。demo.py::
    ...
    def set_rng_seed(self, rng_seed: int = None, random: bool = True, numpy: bool = True,
        ...
        设置模块的随机数种子。由于pytorch还存在cudnn导致的非deterministic的运行，所以一些情况下可能即使seed一样，结果也不一致
            需要在TorchJotter.commit()或TorchJotter.set_log_dir()之后运行才会记录该rng_seed到log中
