"""
TorchJotter是一款集成了自动版本管理和自动日志记录两种功能的 Python 包，它可以帮助你在进行实验时方便地保存当前的代码、参数和结果。
TorchJotter提供给用户的 API 有如下几个：
...
from TorchJotter.fastlog import logger as _logger
from TorchJotter.fastlog.logger import TorchJotterConfig
from TorchJotter.fastgit import Committer, committer as _committer
...
def get_commit_id(file):
    """用户用此命令获取上一次 Git 记录的 id, 期望的使用方法如下::
        ...
        id = TorchJotter.get_commit_id(__file__)
        
    :param file: 以该路径往上寻找.TorchJotter所在文件夹。一般传入__file__即可:
...
def get_fit_id(file):
    """用户用此命令获取上一次 TorchJotter 自动记录 commit 的 id, 期望的使用方法如下::
        
        id = TorchJotter.get_fit_id(__file__)
        
    
    :param file: 以该路径往上寻找.TorchJotter所在文件夹。一般传入__file__即可
    :return: TorchJotter 的上次自动记录的 commit-id 的前七位；错误时返回 `error`
    """
    work_dir = _committer._find_config_file(file)
...
def set_log_dir(log_dir: str, new_log: bool = False):
    ...
    Example::
        ...
        TorchJotter.commit() # 如果commit成功，则不需要设置logs文件夹了
        TorchJotter.set_log_dir('logs/') # 设置log文件夹为'logs/', TorchJotter在每次运行的时候会默认以时间戳的方式在里面生成新的log
        TorchJotter.set_log_dir('logs/log_20190417_140311') # TorchJotter将log继续写入到log_20190417_140311里。
...
def debug(flag=True):
    """
    调用该方法之后，所有的TorchJotter方法都不会产生任何作用。可用于调试代码时避免输出大量无用的信息。

    Example::

        TorchJotter.debug()
        TorchJotter.commit()
        TorchJotter.add_metric(0.3, f1)

    由于有 TorchJotter.debug(), commit() 和 add_metric() 都不会实际执行的。
...
def is_initialized():
    """
    返回TorchJotter是否已经初始化。在使用 TorchJotter.set_log_dir() 时会进行初始化。
...
def finish(status: int = 0, send_to_bot: str = None):
    """
        使用此方法告知 TorchJotter 你的实验已经正确结束。你可以使用此方法来筛选出失败的实验。
...
def add_hyper_in_file(file_path: str=None):
    ...
    如果多次出现，只会记录第一次出现的值。demo.py::
        ...
        TorchJotter.add_hyper_in_file(__file__)  # 会把本python文件的hyper加入进去
        ############hyper
    ...
    :param file_path: 文件路径。如果是读取本python文件中的hyper parameter可以直接TorchJotter.add_hyper_in_file(__file__)
    """
...
def set_rng_seed(rng_seed: int = None, random: bool = True, numpy: bool = True,
    ...
    设置模块的随机数种子。由于pytorch还存在cudnn导致的非deterministic的运行，所以一些情况下可能即使seed一样，结果也不一致
        在TorchJotter.set_log_dir()之后调用本函数将自动记录rng_seed到log中。
...
if 'TorchJotter_FLAG' not in os.environ or os.environ['TorchJotter_FLAG'] == "" or os.environ['TorchJotter_FLAG'] == "NULL":
    os.environ['TorchJotter_FLAG'] = "NULL"
elif os.environ['TorchJotter_FLAG'] == "DEBUG":
    debug()
    print("[TorchJotter] TorchJotter_FLAG is DEBUG. All logging and committing method will not work.")
elif os.environ['TorchJotter_FLAG'] == "NO_COMMIT":
    _logger.no_commit()
    print("[TorchJotter] TorchJotter_FLAG is NO_COMMIT. 'TorchJotter.commit()' will not work.")
else:
    print("[TorchJotter] TorchJotter_FLAG should be 'DEBUG', 'NO_COMMIT' or 'NULL'")
    os.environ['TorchJotter_FLAG'] = "NULL"
