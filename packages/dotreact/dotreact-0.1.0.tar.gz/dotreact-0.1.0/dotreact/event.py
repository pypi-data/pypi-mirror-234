...
from dotreact.utils import console, format
from dotreact.utils.types import ArgsSpec
...
BACKGROUND_TASK_MARKER = "_dotreact_background_task"
...
def set_cookie(key: str, value: str) -> EventSpec:
    ...
    """
    console.deprecate(
        feature_name=f"dr.set_cookie",
        reason="and has been replaced by dr.Cookie, which can be used as a state var",
...
def set_local_storage(key: str, value: str) -> EventSpec:
    ...
    """
    console.deprecate(
        feature_name=f"dr.set_local_storage",
        reason="and has been replaced by dr.LocalStorage, which can be used as a state var",
        deprecation_version="0.3.0",
        removal_version="0.3.1",
    )
    return server_side(
...
def call_event_fn(fn: Callable, arg: Union[Var, ArgsSpec]) -> list[EventSpec]:
    ...
    from dotreact.event import EventHandler, EventSpec
