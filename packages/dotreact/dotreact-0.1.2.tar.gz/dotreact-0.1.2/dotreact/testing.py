"""dotreact.testing - tools for testing dotreact apps."""
...
import dotreact
import dotreact.dotreact
import dotreact.utils.build
import dotreact.utils.exec
import dotreact.utils.prerequisites
import dotreact.utils.processes
from dotreact.state import State, StateManagerMemory, StateManagerRedis
...
class AppHarness:
    """AppHarness executes a dotreact app in-process for testing."""
    ...
    app_instance: Optional[dotreact.App] = None
    ...
    def create(
        ...
        Args:
            ...
            app_source: if specified, the source code from this function or module is used
                ...
                contain a working dotreact app and will be used directly.
    ...
    def _initialize_app(self):
        ...
        if self.app_source is not None:
            ...
            with chdir(self.app_path):
                dotreact.dotreact.init(
                    name=self.app_name,
                    template=dotreact.constants.Templates.Kind.DEFAULT,
                    loglevel=dotreact.constants.LogLevel.INFO,
                )
                self.app_module_path.write_text(source_code)
        with chdir(self.app_path):
            # ensure config and app are reloaded when testing different app
            dotreact.config.get_config(reload=True)
            self.app_module = dotreact.utils.prerequisites.get_app(reload=True)
    ...
    def _start_frontend(self):
        ...
        with chdir(self.app_path):
            ...
            dotreact.utils.build.setup_frontend(self.app_path)

        # Start the frontend.
        self.frontend_process = dotreact.utils.processes.new_process(
            [dotreact.utils.prerequisites.get_package_manager(), "run", "dev"],
            cwd=self.app_path / dotreact.constants.Dirs.WEB,
            env={"PORT": "0"},
            **FRONTEND_POPEN_ARGS,
        )
    ...
    def _wait_frontend(self):
        while self.frontend_url is None:
            ...
            m = re.search(dotreact.constants.Next.FRONTEND_LISTENING_REGEX, line)
            if m is not None:
    ...
    def poll_for_clients(self, timeout: TimeoutType = None) -> dict[str, dotreact.State]:
...
class AppHarnessProd(AppHarness):
    """AppHarnessProd executes a dotreact app in-process for testing.
    ...
    def _run_frontend(self):
        web_root = self.app_path / dotreact.constants.Dirs.WEB / "_static"
        error_page_map = {
    ...
    def _start_backend(self):
        if self.app_instance is None:
            raise RuntimeError("App was not initialized.")
        os.environ[dotreact.constants.SKIP_COMPILE_ENV_VAR] = "yes"
        self.backend = uvicorn.Server(
            uvicorn.Config(
                app=self.app_instance,
                host="127.0.0.1",
                port=0,
                workers=dotreact.utils.processes.get_num_workers(),
            ),
        )
        self.backend.shutdown = self._get_backend_shutdown_handler()
        self.backend_thread = threading.Thread(target=self.backend.run)
    ...
    def _poll_for_servers(self, timeout: TimeoutType = None) -> socket.socket:
        try:
            ...
            os.environ.pop(dotreact.constants.SKIP_COMPILE_ENV_VAR, None)
