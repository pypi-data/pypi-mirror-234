...
from dotreact import constants, model
from dotreact.compiler import templates
from dotreact.config import Config, get_config
from dotreact.utils import console, path_ops, processes
...
def get_default_app_name() -> str:
    ...
    Raises:
        Exit: if the app directory name is dotreact.
    """
    app_name = os.getcwd().split(os.path.sep)[-1].replace("-", "_")

    # Make sure the app is not named "dotreact".
    if app_name == constants.Dotreact.MODULE_NAME:
        console.error(
            f"The app directory cannot be named [bold]{constants.Dotreact.MODULE_NAME}[/bold]."
        )
        raise typer.Exit(1)
...
def initialize_app_directory(app_name: str, template: constants.Templates.Kind):
    """Initialize the app directory on dotreact init.
...
def initialize_web_directory():
    """Initialize the web directory on dotreact init."""
    ...
    # Initialize the dotreact json file.
    init_dotreact_json()
...
def init_dotreact_json():
    """Write the hash of the Dotreact project to a DOTREACT_JSON."""
    # Get a random project hash.
    project_hash = random.getrandbits(128)
    console.debug(f"Setting project hash to {project_hash}.")

    # Write the hash and version to the dotreact json file.
    dotreact_json = {
        "version": constants.Dotreact.VERSION,
        "project_hash": project_hash,
    }
    path_ops.update_json_file(constants.Dotreact.JSON, dotreact_json)


def update_next_config(next_config: str, config: Config) -> str:
    """Update Next.js config from Dotreact config. Is its own function for testing.

    Args:
        next_config: Content of next.config.js.
        config: A dotreact Config object.
...
def install_node():
    """Install fnm and nodejs for use by Dotreact.
...
def install_bun():
    ...
    if unzip_path is None:
        raise FileNotFoundError("Dotreact requires unzip to be installed.")
...
def check_initialized(frontend: bool = True):
    ...
    has_dotreact_dir = not frontend or os.path.exists(constants.Dotreact.DIR)
    has_web_dir = not frontend or os.path.exists(constants.Dirs.WEB)

    # Check if the app is initialized.
    if not (has_config and has_dotreact_dir and has_web_dir):
        console.error(
            f"The app is not initialized. Run [bold]{constants.Dotreact.MODULE_NAME} init[/bold] first."
        )
        raise typer.Exit(1)
    ...
    if frontend and not is_latest_template():
        console.error(
            "The base app template has updated. Run [bold]dotreact init[/bold] again."
        )
        raise typer.Exit(1)
...
def is_latest_template() -> bool:
    ...
    Returns:
        Whether the app is using the latest template.
    """
    if not os.path.exists(constants.Dotreact.JSON):
        return False
    with open(constants.Dotreact.JSON) as f:  # type: ignore
        app_version = json.load(f)["version"]
    return app_version == constants.Dotreact.VERSION
...
def validate_bun():
    ...
    if bun_path != constants.Bun.DEFAULT_PATH:
        ...
        if not bun_version:
            ...
            console.error(
                f"Dotreact requires bun version {constants.Bun.VERSION} or higher to run, but the detected version is "
...
def validate_frontend_dependencies(init=True):
    ...
    Args:
        init: whether running `dotreact init`
    ...
    if not init:
        ...
        # `dotreact init` will install the deps anyway(if applied).
        ...
        if not check_node_version():
            ...
            console.error(
                f"Dotreact requires node version {constants.Node.MIN_VERSION} or higher to run, but the detected version is {node_version}",
            )
            raise typer.Exit(1)
    ...
    if init:
        # we only need bun for package install on `dotreact init`.
        validate_bun()


def initialize_frontend_dependencies():
    """Initialize all the frontend dependencies."""
    # Create the dotreact directory.
    path_ops.mkdir(constants.Dotreact.DIR)
...
def check_db_initialized() -> bool:
    ...
    if get_config().db_url is not None and not Path(constants.ALEMBIC_CONFIG).exists():
        console.error(
            "Database is not initialized. Run [bold]dotreact db init[/bold] first."
        )
        return False
    return True
    ...
    with model.Model.get_db_engine().connect() as connection:
        try:
            if model.Model.alembic_autogenerate(
                ...
                console.error(
                    "Detected database schema changes. Run [bold]dotreact db makemigrations[/bold] "
                    "to generate migration scripts.",
                )
        except CommandError as command_error:
            if "Target database is not up to date." in str(command_error):
                console.error(
                    f"{command_error} Run [bold]dotreact db migrate[/bold] to update database."
                )


def migrate_to_dotreact():
    """Migration from Pynecone to Dotreact."""
    ...
    action = console.ask(
        "Pynecone project detected. Automatically upgrade to Dotreact?",
        choices=["y", "n"],
    )
    if action == "n":
        return
