...
class BenchlingWrapper:
    ...
    def get_sequencing_samples_for_specimens(self, specimen_list: list[list[CustomEntity]]
        ...
        for specimen_for_case in specimen_list:
            for specimen in specimen_for_case:
                ...
                sequencing_sample_list.append(sequencing_sample_for_specimen_ngs_sample)
    ...
    def get_qc_for_sequencing_samples(self, samples_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # to fix tumor2 for cfDNA samples: adding r2 to the end of Data Type row where Data Type == cfDNA-tumo
        qc_df.loc[qc_df['Data Type'] == 'cfDNA-tumo', 'Data Type'] = 'cfDNA-tumor2'
    ...
    def lab_qc_report(self, run_name: str) -> pd.DataFrame:
        ...
        for specimen_name in specimen_names:
            if specimen_name is None:
                ...
                specimen_object = (self.get_entities_by_names(entity_names=[specimen_name]) or [None])[0]
        ...
        for case_name in case_names:
            if case_name is None:
                ...
                case_object = (self.get_entities_by_names(entity_names=[case_name]) or [None])[0]
...
def get_field_from_objects_list(objects: list[CustomEntity | AssayResult | None], field_name: str) -> list:
    ...
    for obj in objects:
        if obj is not None:
            try:
                field_value = get_display_field_value(obj, field_name)
            except KeyError:
                field_value = None
            if field_value is not None and field_value != '':
