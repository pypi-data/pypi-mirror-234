...
def send_to_core_get(path: str, with_auth=True, is_text=False, auth: Optional[AUTH]=None, conn_url: Optional[str]=None) -> Optional[str]:
    host = Config.HOST_PORT if conn_url is None else conn_url
    assert host is not None, "host port not set"
    ...
    check_response(f"{host}{path}", response)
