...
from fundrive.core.base import DriveSnapshot
...
class LanZouDrive(FileSystem):
    ...
    def upload_file(self, file_path="./cache", fid=None, overwrite=False, *args, **kwargs) -> bool:
        ...
        return (self.drive.upload_file(task=task,
                                       file_path=file_path,
                                       folder_id=fid,
                                       callback=clb,
...
class LanZouSnapshot(DriveSnapshot):
    ...
    def delete_outed_version(self, fid):
        datas = self.drive.get_file_list(fid=fid)
    ...
    def update(self, file_path, fid=0, *args, **kwargs):
        gz_path = self._tar_path(file_path)
        with tarfile.open(gz_path, "w:xz") as fw:
            fw.add(file_path, arcname=os.path.basename(file_path))
        self.drive.login()
        self.drive.upload_file(gz_path, fid=fid)
        os.remove(gz_path)
        self.delete_outed_version(fid)

    def download(self, dir_path, url=None, pwd='', *args, **kwargs):
        datas = self.drive.get_file_list(url=url, pwd=pwd)
        if len(datas) == 0:
            return
        datas = sorted(datas, key=lambda x: x['name'], reverse=True)
        ...
        with tarfile.open(tar_path, "r:xz") as fr:
            fr.extractall(path=dir_path)
        os.remove(tar_path)
