...
import logging
import os
from xmind2testcase_wzc.utils import get_xmind_testcase_list, get_absolute_path
...
#open（zentao_file， 'w'， encoding='utf-8'）
...
def xmind_to_zentao_xlsx_file_by_pandas(xmind_file): #xmind导出禅道用例模板的xlsx格式
    ...
    fileheader = ["所属模块", "相关研发需求", "用例标题", "前置条件", "步骤", "预期", "关键词", "优先级", "用例类型", "适用阶段", "用例状态"]  # zd:添加“相关需求”字段
...
def gen_a_testcase_row(testcase_dict):
    ...
    case_status = '正常'
    row = [case_module, case_req, case_title, case_precontion, case_step, case_expected_result, case_keyword,
           case_priority, case_type, case_apply_phase, case_status]
    return row
