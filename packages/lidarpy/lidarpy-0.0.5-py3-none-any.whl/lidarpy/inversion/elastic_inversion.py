...
class Klett:
    ...
    def __init__(self, rangebin: np.array, signal: np.array, molecular_data: xr.Dataset, lidar_ratio: float,
        ...
        self.ref = z_finder(self.rangebin, molecular_reference_region)
    ...
    def _calibration(self, signal):
        if len(self.ref) > 1:
            signal, model, self.fit_parameters = self._calib_strategy(signal=signal.copy(),
                                                                      ...
                                                                      reference=np.arange(*self.ref))

            beta_ref = self._beta['mol'][self.ref[0]] * signal[self.ref[0]] / model[self.ref[0]]
        else:
            signal = signal
            beta_ref = self._beta['mol'][self.ref[0]]

        return beta_ref, signal, self.ref[0]
