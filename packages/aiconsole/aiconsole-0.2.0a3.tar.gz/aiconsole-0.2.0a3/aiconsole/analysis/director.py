...
async def director_analyse(request: Chat) -> AnalysisResponse:
    ...
    async for chunk in gpt_executor.execute(GPTRequest(
        ...
        function_call=EnforcedFunctionCall(name="plan"),
        presence_penalty=2,
    ...
    if (function_call is None):
        raise ValueError(f"Could not find function call in the text: {result}")
    ...
    if not picked_agent:
        _log.error(f"Could not find agent in the text: {result}")
