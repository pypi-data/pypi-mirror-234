...
def gpsvelo(coord, period, sdate=None, data=None, vertical=False, database=False):
    """
    Estimates a linear fit to a gps dataset data for a period defined by
    (sdate-period, sdate)

    input:
        coord: pandas.DataFrame
            Expects a dataframe with station shortname as index
            and columns ["latitude", "longitude", "height"]
        period: int
            number of days to estimate period (sdate-period, sdate)
        sdate: datetime object Default datetime.now()
            defines the end date of the period defined in period.
        data: pandas DataFrame Default reading in dataframe from source
            If usedi, data will be used directly not read from database or file
        veritcal: Boolean Default False
            if vertical is true the vertical component will be evaluted
            east_velo, east_sigma are made 0
            north_velo, north_sigma will reprecent vertical velocities
        database: Boolean Default False
            if true and data == None data will be read from database

    output:
        gpsvelo: pandas.DataFrame
            gps velocities dataframe
            data, dataFrame containing the data if data not 0 the same dataframe
            is pased out as in.
        pb:
            Fit parameters
        pcov
            covarianc matrix of fit
    """

    import logging
    # from geo_dataread.gps_read import read_gps_data, fitdataframe, line, pvel

    # from gps_read import fitdataframe, line, pvel, printvelocity
    import pandas as pd
    ...
    data_period = data[
        data.index.date > (data.index.date[-1] - pd.Timedelta(days=period))
    ]

    pb, pcov = fitdataframe(line, data_period)
    vel, vunc = pvel(pb, pcov)
    ll = coord[["longitude", "latitude"]].to_numpy().flatten()
...
def print_gpsvelo(sta, ll, vel, vertical=False, vfile=None, pheader=False):
    """
