...
class LanZouSnapshot(DriveSnapshot):
    def __init__(self, fid, url, pwd, *args, **kwargs):
        super(LanZouSnapshot, self).__init__(*args, **kwargs)
        self.fid, self.url, self.pwd = fid, url, pwd
        self.drive = LanZouDrive()

    def update(self, file_path):
        gz_path = self._gz_path(file_path)
    ...
    def download(self, dir_path):
        ...
        with tarfile.open(f"{dir_path}/{datas[0]['name']}", "r:xz") as fr:
