...
class Formula(ABC):
        ...
        if not is_valid_collector(array.collector, check_mode):
            ...
            for attrib in ['a5', 'eta0b']:
                if array.collector.is_zero(attrib):
                    r.add_zero_collector_param(array.collector, attrib)
    ...
    def _get_nan_mask_common(self, plant):
        """This method checks sensors common to formulae 1, 2 and 3.
        """
    ...
    def _calc_pc_restrictions_common(self, plant, resampler) -> pd.Series:
        """Checks the operating condition restrictions that are common to PC Formula 1, 2 and 3.
    ...
    def report_problems(self, r: ProblemReport, array: Array, check_mode: AlgoCheckMode) -> ProblemReport:
        ...
        if is_valid_collector(array.collector, check_mode):
            ...
            if array.collector.collector_type != expected_type:
                r.add_wrong_collector_type(array.collector,
            ...
            for attrib in ['a8']:
                if array.collector.is_nonzero(attrib):
                    r.add_nonzero_collector_param(array.collector, attrib)
    ...
    def report_problems(self, r: ProblemReport, array: Array, check_mode: AlgoCheckMode) -> ProblemReport:
        ...
        if is_valid_collector(array.collector, check_mode):
            ...
            if array.collector.concentration_ratio is not None and array.collector.concentration_ratio >= Q(20):
                r.add_generic_slot_problem(array.collector, f'Performance Check formula 2 is only applicable to '
        ...
        if is_valid_collector(array.collector, check_mode):
            ...
            for attrib in ['a8']:
                if array.collector.is_zero(attrib):
                    r.add_zero_collector_param(array.collector, attrib)
            if array.collector.concentration_ratio is not None and array.collector.concentration_ratio < Q(20):
                r.add_generic_slot_problem(array.collector, f'Performance Check formula 3 is only applicable to '
                                                            f'concentrating collectors with a concentration_ratio >= 20. '
                                                            f'This collector has a concentration_ratio of '
                                                            f'{array.collector.concentration_ratio.m:.2f}.')
