...
def delete_measure_data(plant_id: int,
    ...
    def get_start_end_tuples(start_, end_):
        pd_dates = [pd.to_datetime(x) for x in [start_, end_]]
        return ({'timestamp': x.to_pydatetime(), 'year': x.year, 'quarter': x.quarter} for x in pd_dates)

    plant = crd.get_plants(sess, plant_id)
    plant.context.delete_data(start, end)
