...
class ProvisionClient(paho.Client):
    ...
    def __on_connect(self, client, userdata, flags, rc):  # Callback for connect
        if rc == 0:
            log.info("[Provisioning client] Connected to WaveletThings ")
            ...
            log.info("[Provisioning client] Cannot connect to WaveletThings!, result: %s" % RESULT_CODES[rc])

    def __on_message(self, client, userdata, msg):
        decoded_payload = msg.payload.decode("UTF-8")
        log.info("[Provisioning client] Received data from WaveletThings: %s" % decoded_payload)
    ...
    def provision(self):
        log.info("[Provisioning client] Connecting to WaveletThings")
...
class WTDeviceMqttClient:
            ...
            if not self.__timeout_queue.empty():
                ...
                if item is not None:
                    ...
                    if callback is not None:
                        callback(self, None,
                                 WTTimeoutException("Timeout while waiting for a reply from WaveletThings!"))
            else:
                time.sleep(0.01)
    ...
    def claim(self, secret_key, duration=30000):
        ...
        info = WTPublishInfo(self._client.publish(CLAIMING_TOPIC, dumps(claiming_request), qos=self.quality_of_service))
        return info
