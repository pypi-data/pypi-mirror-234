...
class App(tk.Frame):
    def __init__(self, master):
        ...
        self.l3.pack()
        self.l3=Label(master,text="gamma, chi, betaprime, pareto, erlang, exponential, lognormal, arcsine, fatiguelife, weibull.")
        self.l3.pack()
...
def main(N_observations,max_xs,theta_true,k_true, distribution):
    ...
    if distribution == "gamma":
        F = gamma.cdf(xs, a = k_true+0.1, loc = 0, scale = theta_true)
        Fb = biased_F(F,xs)
        axis[0,0].plot(xs, F, color = "orange", label = r"cdf $F$ debiased hidden $X \sim$ " + "Ga({0},{1})".format(k_true+0.1,theta_true))
        ...
        F = gamma.cdf(xs, a = k_true+2, loc = 0, scale = theta_true)
        Fb = biased_F(F,xs)
        axis[0,0].plot(xs, F, color = "orange", label = r"cdf $F$ debiased hidden $X \sim$ " + "Erl({0},{1})".format(k_true+2,theta_true))
        axis[0,0].plot(xs, Fb, color = "red", label = r"cdf $F^{(b)}$ biased hidden $X^b$")
        axis[0,0].set_xlabel("x", fontsize = 12)
        axis[0,0].legend(loc = "lower right", fontsize=13)
    elif distribution == "exponential":
        F = gamma.cdf(xs, a = k_true-1, loc = 0, scale = theta_true)
        Fb = biased_F(F,xs)
        axis[0,0].plot(xs, F, color = "orange", label = r"cdf $F$ debiased hidden $X \sim$ " + "Exp({0})".format(theta_true))
        axis[0,0].plot(xs, Fb, color = "red", label = r"cdf $F^{(b)}$ biased hidden $X^b$")
        axis[0,0].set_xlabel("x", fontsize = 12)
        axis[0,0].legend(loc = "lower right", fontsize=13)
        ...
        F = betaprime.cdf(xs, a = k_true-1, b = theta_true+1.5)
        Fb = biased_F(F,xs)
        axis[0,0].plot(xs, F, color = "orange", label = r"cdf $F$ debiased hidden $X \sim$ " + "Bepr({0},{1})".format(k_true-1,theta_true+1.5))
        axis[0,0].plot(xs, Fb, color = "red", label = r"cdf $F^{(b)}$ biased hidden $X^b$")
        axis[0,0].set_xlabel("x", fontsize = 12)
        axis[0,0].legend(loc = "lower right", fontsize=13)
    elif distribution == "pareto":
        F = pareto.cdf(xs, b = theta_true+2)
        Fb = biased_F(F,xs)
        axis[0,0].plot(xs, F, color = "orange", label = r"cdf $F$ debiased hidden $X \sim$ " + "Pareto({0})".format(theta_true+2))
        axis[0,0].plot(xs, Fb, color = "red", label = r"cdf $F^{(b)}$ biased hidden $X^b$")
        axis[0,0].set_xlabel("x", fontsize = 12)
        axis[0,0].legend(loc = "lower right", fontsize=13)
    ...
    if distribution == "gamma":        
        ...
        axis[1,1].plot(xs, F, color = "red", label = r"cdf $F$, $X \sim$ " + "Ga({0},{1})".format(k_true+0.1,theta_true))
        ...
        axis[1,1].step(support_lcm[1:],estimator_F,where = "post", label = r"Isotonic estimator of $F$")
        axis[1,1].plot(xs, F, color = "red", label = r"cdf $F$, $X \sim$ " + "Erl({0},{1})".format(k_true-1,theta_true))
        axis[1,1].set_xlabel("x", fontsize = 12)
        axis[1,1].legend(loc = "lower right", fontsize=16)
        plt.savefig('estimator.svg', format='svg', dpi=1200)
        plt.show()
        ...
        axis[1,1].step(support_lcm[1:],estimator_F,where = "post", label = r"Isotonic estimator of $F$")
        axis[1,1].plot(xs, F, color = "red", label = r"cdf $F$, $X \sim$ " + "Exp({0})".format(theta_true))
        axis[1,1].set_xlabel("x", fontsize = 12)
        axis[1,1].legend(loc = "lower right", fontsize=16)
        plt.savefig('estimator.svg', format='svg', dpi=1200)
        plt.show()
        ...
        axis[1,1].step(support_lcm[1:],estimator_F,where = "post", label = r"Isotonic estimator of $F$")
        axis[1,1].plot(xs, F, color = "red", label = r"cdf $F$ hidden $X \sim$ " + "Bepr({0},{1})".format(k_true-1,theta_true+1.5))
        axis[1,1].set_xlabel("x", fontsize = 12)
        axis[1,1].legend(loc = "lower right", fontsize=16)
        plt.savefig('estimator.svg', format='svg', dpi=1200)
        plt.show()
        ...
        axis[1,1].step(support_lcm[1:],estimator_F,where = "post", label = r"Isotonic estimator of $F$")
        axis[1,1].plot(xs, F, color = "red", label = r"cdf $F$ hidden $X \sim$ " + "Pareto({0})".format(theta_true+2))
        axis[1,1].set_xlabel("x", fontsize = 12)
        axis[1,1].legend(loc = "lower right", fontsize=16)
        plt.savefig('estimator.svg', format='svg', dpi=1200)
        plt.show()
