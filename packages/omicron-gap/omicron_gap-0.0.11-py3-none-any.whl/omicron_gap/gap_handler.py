...
def main():
    ...
    programs, not_found = which_programs(["omicron_run.sh", "python", "omicron-find-gaps",  "condor_submit_dag"])
    programs["subdag_creator"] = mydir / 'gap-subdag-create.py'
    goterr = False
    ...
    for group in proc_groups:
        ...
        find_args = f'{programs["python"]} {programs["omicron_find_gaps"]} {out_verbosity} --group {group} --ifo {ifo} ' \
        ...
        find_job.add_classad('executable', f'{programs["omicron_run.sh"]}')
        ...
        fill_job.add_classad('executable', f'{programs["omicron_run.sh"]}')
        ...
        subdag_cr_args = f'{programs["python"]} {programs["subdag_creator"]} -vvv ' \
                         f'--group {group} --inpath {output_grp_dir}  ' \
                         f'--outpath' \
                         f' {subdag_path}'
        create_subdag_script = OmicronScript(is_post=True, parent=fill_job, script=programs["omicron_run.sh"],
