...
class UzMorphAnalyser:
    ...
    def __processing(self, word: str, pos: str = None, is_lemmatize: bool = False, multi_item: bool = False):
        ...
        def stem_find(self, word: str, pos: str, position: int = 1): # position = 1 adi
            ...
            if word.casefold().startswith(item_ex['stem']):
                ...
                for i_affixes in affixes_temp:  # qushimchani affixes dan qidirib topib, undagi malumotlarni olamiz
                    if full_affix in i_affixes["allomorphs"] and i_affixes['pos'] == item_ex['pos']:
                        i_affixes['stem'], i_affixes['lemma'], i_affixes['affixed'] = stem, stem, full_affix
                        result_items.append(i_affixes)
                ...
                if word.casefold() == item_ex['stem']:
                    result_items.append({'stem': word, 'lemma': word, 'affixed': '', 'pos': item_ex['pos']})
        ...
        # 5-step
        affixes_temp = affixes
        if is_lemmatize: # bu yerga lemmatize va analyze da kiradi
        ...
        result_items = result_items + stem_find(self, word, pos) # stem_find() ni ichidan topib yasalgan listni bundan oldingi qadamlarda yasalgan list ustiga qushamiz, qaysidir birlari bush buladi katta ehtimo l bilan
...
while (True):
    ...
    print(obj.stem(s))
    print(obj.lemmatize(s))
    res_analyze = obj.analyze(s)
