...
def retrieve_bins_dict(df, features, nbins=1000, bin_min_pct=0.04):
    ...
    for col in features:
        ...
        # numerical bins generation
            ...
            base_bin = pd.cut(df[col].dropna(), cuts, right=True, duplicates="drop")
...
def map_bins_dict(df, meta_dict, features):
    ...
    for col in features:
        ...
        if meta_dict[col]["type"] == "numerical":
            for x in [y for y in meta_dict[col].keys() if y not in ["type", "min_val", "max_val", "missing_values"]]:
        ...
        if meta_dict[col]["type"] == "categorical":
            for x in [y for y in meta_dict[col].keys() if y not in ["type", "missing_values"]]:
