            ...
            while attempts < max_attempts:
                try:
                    ...
                    if attempts < max_attempts:
                        ...
                        raise Exception(f"Max attempts reached ({max_attempts})")
        ...
        try:
            ...
            if result_dataframe != '':
                ...
                for ele in ["fault_start_time", "fault_end_time", "analy_time", "created_at", "updated_at"]:
                    df[ele] = df[ele].map(lambda x: datetime.datetime.fromtimestamp(x/1000, new_timezone) if pd.notna(x) else x)
