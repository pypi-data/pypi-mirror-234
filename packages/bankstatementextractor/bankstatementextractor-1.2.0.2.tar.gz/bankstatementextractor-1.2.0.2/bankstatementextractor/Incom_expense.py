...
labels = pd.read_csv('./bankstatementextractor/models/lean/labels')['0']
...
class Incom_expense:   
    ...
    def income_detection(self,res_json):
            ...
            average_income=0
            model_name = 'places'
            id = pd.Series(pd.to_numeric(os.listdir('./bankstatementextractor/models/'+model_name), errors='coerce'),).dropna(

            ).sort_values(ascending=False).iloc[0]  

            places_categories = pd.read_csv('./bankstatementextractor/models/categories.csv',)['0']
            # categories
            places_model = tf.keras.models.load_model('./bankstatementextractor/models/'+model_name+'/'+str(id))
            ...
            # .sort_values()
            words_categories = pd.read_csv('./bankstatementextractor/models/categories.csv',)['0']
            # categories
            words_model = tf.keras.models.load_model('./bankstatementextractor/models/'+model_name+'/'+str(id))
        ...
        # try:
            ...
            if len(groups) > 0 and groups:
                ...
                qt = load(open('./bankstatementextractor/qt.pkl', 'rb'))
                ...
                model = tf.keras.models.load_model(
                    './bankstatementextractor/models/lean/0.9384675110399379')
