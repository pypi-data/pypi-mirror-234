...
class Wireguard:
    ...
    def adddevice(self):
        deivie_name = Prompt.ask("[bold green]Enter the device name :Desktop_Computer:  [/bold green]")
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to add device. Status code: {response.status_code}")
            print(response.text)
    def addip(self):
        ...
        # Use subprocess to modify 'wg0.conf' with sudo privileges
        try:
            sed_command = f"sudo sed -i '/^Address/s/.*/Address = {allowed_ips}/' /etc/wireguard/wg0.conf"
            subprocess.run(sed_command, shell=True, check=True, executable="/bin/bash")
