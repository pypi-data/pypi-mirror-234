            ...
            for i in range(9, len(self.data.columns)):
                ...
                for j in range(len(self.data)):
                    ...
                    if check_list:
                        ...
                        self.data.iat[j, i] = -1
            out = '{}.geno'.format(args.vcf.split('.')[0])
            self.data.to_csv(out, sep='\t', index=False)
            print(time_stamp(), 'Done.', flush=True)
        
        elif args.geno != 'x':
            print(time_stamp(), 'Extracting variants data.', flush=True)
            self.data = pd.read_csv(args.geno, sep='\t')
            self.vcf_col = self.data.columns
            print(time_stamp(), 'Done.', flush=True)
        else:
            print(time_stamp(), 'Please set VCF of geno file.', flush=True)
            sys.exit(1)
