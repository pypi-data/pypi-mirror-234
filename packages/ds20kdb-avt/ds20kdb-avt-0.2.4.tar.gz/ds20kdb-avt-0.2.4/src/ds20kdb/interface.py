...
class Database(auth.Authentication):
    ...
    def get_vpcb_asic_pid_from_qrcode(self, qrcode):
        ...
        try:
            vpcb_pid = self.get_vpcb_pid(qrcode).data
        except AttributeError:
            pass
        else:
    ...
    def get_vpcb_pid(self, qrcode):
        ...
        response = self.get('vpcb', qrcode=qrcode)
    ...
    def get_tile_pid_from_qrcode(self, qrcode):
        ...
        try:
            pcb_pid = self.get('pcb', qrcode=qrcode).data.pcb_pid.values[-1]
        except (AttributeError, IndexError):
            return None
    ...
    def get_vtile_pid_from_qrcode(self, qrcode):
        ...
        try:
            vpcb_asic_pid = self.get_vpcb_asic_pid_from_qrcode(qrcode).data
        except IndexError:
            pass
        else:
    ...
    def get_qrcode_from_vtile_pid(self, vtile_pid):
        ...
        ----------------------------------------------------------------------
        """
        # Perform some type checking. vtile_pid should be an integer, if a
        # float is supplied and is directly convertable to an int (e.g. 57.0)
        # convert to int so the db lookup doesn't fail.
        #
        # Generate a warning if the float isn't equivalent to an int, so
        # (1) we avoid an off-by-one error, and (2) it's clear to the user
        # why the call has failed.
        if isinstance(vtile_pid, float):
            if vtile_pid.is_integer():
                vtile_pid = int(vtile_pid)
            else:
                logging.debug(
                    'get_qrcode_from_vtile_pid: integer expected, received "%s"',
                    vtile_pid
                )
                return None
