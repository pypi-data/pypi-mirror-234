...
from dotserve.utils import console, format
from dotserve.utils.types import ArgsSpec
...
BACKGROUND_TASK_MARKER = "_dotserve_background_task"
...
def set_cookie(key: str, value: str) -> EventSpec:
    ...
    """
    console.deprecate(
        feature_name=f"ds.set_cookie",
        reason="and has been replaced by ds.Cookie, which can be used as a state var",
...
def set_local_storage(key: str, value: str) -> EventSpec:
    ...
    """
    console.deprecate(
        feature_name=f"ds.set_local_storage",
        reason="and has been replaced by ds.LocalStorage, which can be used as a state var",
        deprecation_version="0.3.0",
        removal_version="0.3.1",
    )
    return server_side(
...
def call_event_fn(fn: Callable, arg: Union[Var, ArgsSpec]) -> list[EventSpec]:
    ...
    from dotserve.event import EventHandler, EventSpec
