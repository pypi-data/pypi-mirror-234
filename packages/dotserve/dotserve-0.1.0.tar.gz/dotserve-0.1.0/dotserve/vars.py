...
from dotserve.utils import console, format, serializers, types
...
class Var(ABC):
    ...
    def __bool__(self) -> bool:
        ...
        raise TypeError(
            ...
            "Instead use `ds.cond` and bitwise operators `&` (and), `|` (or), `~` (invert)."
        )
    ...
    def __iter__(self) -> Any:
        ...
        """
        raise TypeError(
            f"Cannot iterate over Var {self.full_name!r}. Instead use `ds.foreach`."
        )
...
def get_local_storage(key: Var | str | None = None) -> BaseVar:
    ...
    """
    console.deprecate(
        feature_name=f"ds.get_local_storage",
