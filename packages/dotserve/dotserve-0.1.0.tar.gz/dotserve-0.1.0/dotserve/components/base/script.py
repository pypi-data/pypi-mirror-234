...
from dotserve.event import EventChain
from dotserve.utils import console
...
class Script(Component):
    ...
    Note that this component differs from dotserve.components.base.document.NextScript
    in that it is intended for use with custom and user-defined scripts.

    It also differs from dotserve.components.base.link.ScriptTag, which is the plain
...
def client_side(javascript_code) -> Var[EventChain]:
    ...
    """
    console.deprecate(
        feature_name="ds.client_side",
        reason="and has been replaced by ds.call_script, which can be used from backend EventHandler too",
