...
from dotserve.admin import AdminDash
from dotserve.base import Base
from dotserve.compiler import compiler
from dotserve.compiler import utils as compiler_utils
from dotserve.components import connection_modal
from dotserve.components.component import Component, ComponentStyle
from dotserve.components.layout.fragment import Fragment
from dotserve.components.navigation.client_side_routing import (
    Default404Page,
    wait_for_client_redirect,
)
from dotserve.config import get_config
from dotserve.event import Event, EventHandler, EventSpec
from dotserve.middleware import HydrateMiddleware, Middleware
from dotserve.model import Model
from dotserve.page import (
    DECORATED_PAGES,
)
from dotserve.route import (
    catchall_in_route,
    catchall_prefix,
    get_route_args,
...
from dotserve.utils import console, format, prerequisites, types
    ...
    def __init__(self, *args, **kwargs):
        ...
        Raises:
            ValueError: If the event namespace is not provided in the config.
                        Also, if there are multiple client subclasses of ds.State(Subclasses of ds.State should consist
        ...
        # Special case to allow test cases have multiple subclasses of ds.State.
        if not is_testing_env:
            # Only the default state and the client state should be allowed as subclasses.
            if len(state_subclasses) > 2:
                raise ValueError(
                    "ds.State has been subclassed multiple times. Only one subclass is allowed"
                )

            # verify that provided state is valid
            if self.state not in [DefaultState, inferred_state]:
                console.warn(
                    f"Using substate ({self.state.__name__}) as root state in `ds.App` is currently not supported."
        ...
        if admin_dash and admin_dash.models:
            ...
            admin = (
                ...
                if admin_dash.admin
                    ...
                    title="Dotserve Admin Dashboard",
                    logo_url="https://dotagent.dev/Dotserve.svg",
                )
            )
