"""dotserve.testing - tools for testing dotserve apps."""
...
import dotserve
import dotserve.dotserve
import dotserve.utils.build
import dotserve.utils.exec
import dotserve.utils.prerequisites
import dotserve.utils.processes
from dotserve.state import State, StateManagerMemory, StateManagerRedis
...
class AppHarness:
    """AppHarness executes a dotserve app in-process for testing."""
    ...
    app_instance: Optional[dotserve.App] = None
    ...
    def create(
        ...
        Args:
            ...
            app_source: if specified, the source code from this function or module is used
                ...
                contain a working dotserve app and will be used directly.
    ...
    def _initialize_app(self):
        ...
        if self.app_source is not None:
            ...
            with chdir(self.app_path):
                dotserve.dotserve.init(
                    name=self.app_name,
                    template=dotserve.constants.Templates.Kind.DEFAULT,
                    loglevel=dotserve.constants.LogLevel.INFO,
                )
                self.app_module_path.write_text(source_code)
        with chdir(self.app_path):
            # ensure config and app are reloaded when testing different app
            dotserve.config.get_config(reload=True)
            self.app_module = dotserve.utils.prerequisites.get_app(reload=True)
    ...
    def _start_frontend(self):
        ...
        with chdir(self.app_path):
            ...
            dotserve.utils.build.setup_frontend(self.app_path)

        # Start the frontend.
        self.frontend_process = dotserve.utils.processes.new_process(
            [dotserve.utils.prerequisites.get_package_manager(), "run", "dev"],
            cwd=self.app_path / dotserve.constants.Dirs.WEB,
            env={"PORT": "0"},
            **FRONTEND_POPEN_ARGS,
        )
    ...
    def _wait_frontend(self):
        while self.frontend_url is None:
            ...
            m = re.search(dotserve.constants.Next.FRONTEND_LISTENING_REGEX, line)
            if m is not None:
    ...
    def poll_for_clients(self, timeout: TimeoutType = None) -> dict[str, dotserve.State]:
...
class AppHarnessProd(AppHarness):
    """AppHarnessProd executes a dotserve app in-process for testing.
    ...
    def _run_frontend(self):
        web_root = self.app_path / dotserve.constants.Dirs.WEB / "_static"
        error_page_map = {
    ...
    def _start_backend(self):
        if self.app_instance is None:
            raise RuntimeError("App was not initialized.")
        os.environ[dotserve.constants.SKIP_COMPILE_ENV_VAR] = "yes"
        self.backend = uvicorn.Server(
            uvicorn.Config(
                app=self.app_instance,
                host="127.0.0.1",
                port=0,
                workers=dotserve.utils.processes.get_num_workers(),
            ),
        )
        self.backend.shutdown = self._get_backend_shutdown_handler()
        self.backend_thread = threading.Thread(target=self.backend.run)
    ...
    def _poll_for_servers(self, timeout: TimeoutType = None) -> socket.socket:
        try:
            ...
            os.environ.pop(dotserve.constants.SKIP_COMPILE_ENV_VAR, None)
