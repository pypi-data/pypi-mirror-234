"""Dotserve CLI to create, run, and deploy apps."""
...
from dotserve.utils import build, console, exec, prerequisites, processes, telemetry
...
def version(value: bool):
    ...
    """
    if value:
        console.print(constants.Dotserve.VERSION)
        raise typer.Exit()
...
def init(
    ...
    """Initialize a new Dotserve app in the current directory."""
    # Set the log level.
    ...
    # Migrate Pynecone projects to Dotserve.
    prerequisites.migrate_to_dotserve()
...
def run(
    ...
    console.rule("[bold]Starting Dotserve App")
...
def deploy(
    ...
    """Deploy the app to the Dotserve hosting service."""
    # Set the log level.
    console.set_log_level(loglevel)
...
def db_init():
    ...
    if Path(constants.ALEMBIC_CONFIG).exists():
        console.error(
            ...
            "[bold]dotserve db makemigrations[/bold] to create schema change "
            "scripts and [bold]dotserve db migrate[/bold] to apply migrations "
