...
from dotserve import constants, model
from dotserve.compiler import templates
from dotserve.config import Config, get_config
from dotserve.utils import console, path_ops, processes
...
def get_default_app_name() -> str:
    ...
    Raises:
        Exit: if the app directory name is dotserve.
    """
    app_name = os.getcwd().split(os.path.sep)[-1].replace("-", "_")

    # Make sure the app is not named "dotserve".
    if app_name == constants.Dotserve.MODULE_NAME:
        console.error(
            f"The app directory cannot be named [bold]{constants.Dotserve.MODULE_NAME}[/bold]."
        )
        raise typer.Exit(1)
...
def initialize_app_directory(app_name: str, template: constants.Templates.Kind):
    """Initialize the app directory on dotserve init.
...
def initialize_web_directory():
    """Initialize the web directory on dotserve init."""
    ...
    # Initialize the dotserve json file.
    init_dotserve_json()
...
def init_dotserve_json():
    """Write the hash of the Dotserve project to a DOTSERVE_JSON."""
    # Get a random project hash.
    project_hash = random.getrandbits(128)
    console.debug(f"Setting project hash to {project_hash}.")

    # Write the hash and version to the dotserve json file.
    dotserve_json = {
        "version": constants.Dotserve.VERSION,
        "project_hash": project_hash,
    }
    path_ops.update_json_file(constants.Dotserve.JSON, dotserve_json)


def update_next_config(next_config: str, config: Config) -> str:
    """Update Next.js config from Dotserve config. Is its own function for testing.

    Args:
        next_config: Content of next.config.js.
        config: A dotserve Config object.
...
def install_node():
    """Install fnm and nodejs for use by Dotserve.
...
def install_bun():
    ...
    if unzip_path is None:
        raise FileNotFoundError("Dotserve requires unzip to be installed.")
...
def check_initialized(frontend: bool = True):
    ...
    has_dotserve_dir = not frontend or os.path.exists(constants.Dotserve.DIR)
    has_web_dir = not frontend or os.path.exists(constants.Dirs.WEB)

    # Check if the app is initialized.
    if not (has_config and has_dotserve_dir and has_web_dir):
        console.error(
            f"The app is not initialized. Run [bold]{constants.Dotserve.MODULE_NAME} init[/bold] first."
        )
        raise typer.Exit(1)
    ...
    if frontend and not is_latest_template():
        console.error(
            "The base app template has updated. Run [bold]dotserve init[/bold] again."
        )
        raise typer.Exit(1)
...
def is_latest_template() -> bool:
    ...
    Returns:
        Whether the app is using the latest template.
    """
    if not os.path.exists(constants.Dotserve.JSON):
        return False
    with open(constants.Dotserve.JSON) as f:  # type: ignore
        app_version = json.load(f)["version"]
    return app_version == constants.Dotserve.VERSION
...
def validate_bun():
    ...
    if bun_path != constants.Bun.DEFAULT_PATH:
        ...
        if not bun_version:
            ...
            console.error(
                f"Dotserve requires bun version {constants.Bun.VERSION} or higher to run, but the detected version is "
...
def validate_frontend_dependencies(init=True):
    ...
    Args:
        init: whether running `dotserve init`
    ...
    if not init:
        ...
        # `dotserve init` will install the deps anyway(if applied).
        ...
        if not check_node_version():
            ...
            console.error(
                f"Dotserve requires node version {constants.Node.MIN_VERSION} or higher to run, but the detected version is {node_version}",
            )
            raise typer.Exit(1)
    ...
    if init:
        # we only need bun for package install on `dotserve init`.
        validate_bun()


def initialize_frontend_dependencies():
    """Initialize all the frontend dependencies."""
    # Create the dotserve directory.
    path_ops.mkdir(constants.Dotserve.DIR)
...
def check_db_initialized() -> bool:
    ...
    if get_config().db_url is not None and not Path(constants.ALEMBIC_CONFIG).exists():
        console.error(
            "Database is not initialized. Run [bold]dotserve db init[/bold] first."
        )
        return False
    return True
    ...
    with model.Model.get_db_engine().connect() as connection:
        try:
            if model.Model.alembic_autogenerate(
                ...
                console.error(
                    "Detected database schema changes. Run [bold]dotserve db makemigrations[/bold] "
                    "to generate migration scripts.",
                )
        except CommandError as command_error:
            if "Target database is not up to date." in str(command_error):
                console.error(
                    f"{command_error} Run [bold]dotserve db migrate[/bold] to update database."
                )


def migrate_to_dotserve():
    """Migration from Pynecone to Dotserve."""
    ...
    action = console.ask(
        "Pynecone project detected. Automatically upgrade to Dotserve?",
        choices=["y", "n"],
    )
    if action == "n":
        return
    ...
    updates = {
        "Pynecone": "Dotserve",
        "pynecone as pc": "dotserve as ds",
        "pynecone.io": "dotserve.dev",
        "pynecone": "dotserve",
        "pc.": "ds.",
        "pcconfig": "dsconfig",
    }
