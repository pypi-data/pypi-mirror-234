...
class ExtraAssetsPanel(Panel):
    ...
    def draw(self, context):
        ...
        for i, extra_asset in enumerate(extra_assets_list):
            ...
            # Todo: Do we need to give the option of deleting the extra asset row?
            #row.operator("custom.remove_extra_asset", text="X").index = i
...
class SubmitOperator(Operator):
    ...
    def execute(self, context):
        ...
        #Todo: Finish the progress tabs: Add fourth row
...
def register():
    ...
    bpy.types.Scene.output_folder = bpy.props.StringProperty(
        name="Output Folder",
        description="Path to the output folder for rendered images",
        default="",  # You can set a default value here
        subtype='DIR_PATH',  # Use 'DIR_PATH' subtype for directory paths
    )
...
def unregister():
    ...
    del bpy.types.Scene.extra_assets_list
