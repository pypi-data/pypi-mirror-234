...
class ATACDataset(object):
    ...
    def quality_control(self, min_features: int = 1000, max_features: int = 50000,
                        min_percent: Optional[int] = None, min_cells: Optional[int] = None):
        ...
        if min_percent is not None:
            self.array_celltype = np.unique(np.array(adata_atac.obs['celltype']))
            df_count = pd.DataFrame(adata_atac.X.toarray(), index=adata_atac.obs.index,
                                    columns=adata_atac.var.index)
            df_percent = pd.DataFrame(
                np.full(shape=(df_count.shape[1], len(self.array_celltype)), fill_value=0),
                index=adata_atac.var.index, columns=self.array_celltype)
            for celltype_qc in self.array_celltype:
                sub_count = df_count.loc[adata_atac.obs['celltype'] == celltype_qc, :]
                sub_percent = np.sum(sub_count != 0, axis=0) / sub_count.shape[0]
                df_percent[celltype_qc] = sub_percent
            df_percent_max = np.max(df_percent, axis=1)
            sel_peaks = df_percent.index[df_percent_max > min_percent]
    ...
    def deepen_atac(self, num_peak=50000, num_pc=50, num_cell_merge=10):
        if sparse.issparse(self.adata.X):
            df_atac = pd.DataFrame(self.adata.X.astype(np.float32).toarray(),
                                   index=self.adata.obs.index, columns=self.adata.var.index)
        else:
            df_atac = pd.DataFrame(self.adata.X.astype(np.float32),
                                   index=self.adata.obs.index, columns=self.adata.var.index)
        adata_atac_sample_cluster = self.adata.copy()
        sc.pp.normalize_total(adata_atac_sample_cluster)
        sc.pp.log1p(adata_atac_sample_cluster)
        sc.pp.highly_variable_genes(adata_atac_sample_cluster,
                                    n_top_genes=num_peak, flavor='seurat')
        adata_atac_sample_cluster = \
            adata_atac_sample_cluster[:, adata_atac_sample_cluster.var.highly_variable]
        sc.pp.scale(adata_atac_sample_cluster, max_value=10)
        sc.tl.pca(adata_atac_sample_cluster, svd_solver='arpack', n_comps=num_pc)
        sc.pp.neighbors(adata_atac_sample_cluster, n_neighbors=num_cell_merge, n_pcs=num_pc)
        ...
        for cell_atac in list(adata_atac_sample_cluster.obs.index):
            ...
            cell_sample_atac = \
                cell_atac + list(adata_atac_sample_cluster.obs.iloc[cell_neighbor_idx, :].index[
                                 0:(num_cell_merge-1)])

            sub_atac = np.array(df_atac.loc[cell_sample_atac, :]).astype(np.float32)
            list_atac.append(np.sum(sub_atac, axis=0).reshape((1, sub_atac.shape[1])))

        array_atac = np.concatenate(list_atac, axis=0)
        array_atac = sparse.csr_matrix(array_atac)
        # array_atac = sparse.lil_matrix(array_atac)
    ...
    def add_promoter(self, file_tss, flank_proximal=2000):
        ...
        subprocess.run(
            f"bedtools intersect -a {self.file_peaks_sort} -b {file_proximal} -wao "
