...
class MsgJournalPublisher(MsgPublisher):
    ...
    async def log(self,
        ...
        return await self.publish_journal_operation(entry, op='publish')

    async def publish_journal_operation(self, entry: JournalEntry, op: str) -> JournalEntry:
        ...
        dentry['op'] = op
        await self.publish(dentry, meta={'message_type': f'journal.{op}'})
        return entry
