...
class _SolClientNativeRxMsgDispatchFuncInfo(Structure) \
    ...
    _fields_ = [
        ("dispatch_type", ctypes.c_uint32),  # The type of dispatch described
        #("callback_p", ctypes.CFUNCTYPE(ctypes.c_int, c_void_p, c_void_p, py_object)),  # An application-defined
        ("callback_p", ctypes.c_void_p), # a native pointer callback, Note cyptes._FuncPtr is not compatibile for a native callback pointer
        # callback function; may be NULL if there is no callback.
        ("user_p", c_void_p),
        # A user pointer to return with the callback; must be NULL if callback_p is NULL.
        ("rffu", c_void_p)  # Reserved for Future use; must be NULL
...
def __get_dispatch_filter_func(filterType:int):
    callback_p = ctypes.c_void_p(None)
    rc = solace.CORE_LIB.solClient_topicDispatchFilter_getFilterCallback(ctypes.c_uint64(filterType), byref(callback_p))
    if rc != SOLCLIENT_OK:
...
def topic_unsubscribe_with_dispatch(session_p, subscription, func_info_p, flags:int = SOLCLIENT_SUBSCRIBE_FLAGS_WAITFORCONFIRM):
