...
from solace.messaging.utils._solace_utilities import _SolaceThread, QueueShutdown
...
class _CacheEventThread(_SolaceThread):
    def __init__(self, owner_info, owner_logger: 'logging.Logger',
        receiver: '_DirectMessageReceiver', *args, **kwargs):
        super().__init__(owner_info, owner_logger, *args, **kwargs)
        self._process_cache_event = receiver.cache_event_processing_callback
        self._cache_event_queue = receiver.cache_event_queue
        self._stop_event = receiver.stop_event

    def _on_join(self):
        self._stop_event.set()
        self._cache_event_queue.shutdown()

    def run(self):  # pylint: disable=missing-function-docstring,too-many-branches
        if self.adapter.isEnabledFor(logging.DEBUG):
            self.adapter.debug('THREAD: [%s] started', type(self).__name__)

        # Initialize locals in case _stop_event is already set
        cache_event_info = None
        exception = None

        while not self._stop_event.is_set():
            # Reset locals at the beginning of each iteration to flush previous entries
            cache_event_info = None
            exception = None
            try:
                # retrieve event from queue
                cache_event_info, exception = self._cache_event_queue.get()
            except QueueShutdown:
                break

            # This should happen everytime a cache response is received while the cache event queue is not shutdown
            self._process_cache_event(cache_event_info, exception)

        # This should only happen on terminate, when the cache event queue is shutdown; or,
        # if the thread is forcefully joined.
        drained_cache_events = self._cache_event_queue.drain()
        for cache_event_info, exception in drained_cache_events:
            self._process_cache_event(cache_event_info, exception)
