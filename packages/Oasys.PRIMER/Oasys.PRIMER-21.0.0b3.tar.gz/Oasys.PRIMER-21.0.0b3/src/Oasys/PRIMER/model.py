...
class ModelType(type):
    _consts = {'ABAQUS', 'CENTRE_AT_COFG', 'COMPRESS_KEEP', 'COMPRESS_OFF', 'COMPRESS_ON', 'DISCARD_MASTER_CLASH', 'DISCARD_SLAVE_CLASH', 'GLOBAL_AXES', 'IGES', 'IGNORE_CLASH', 'INCREASE_MASTER_ALWAYS', 'INCREASE_MASTER_CLASH', 'INCREASE_SLAVE_ALWAYS', 'INCREASE_SLAVE_CLASH', 'INDIVIDUAL_GZIP', 'INDIVIDUAL_ZIP', 'KEEP_ORIGINAL', 'LOCAL_AXES', 'LSDYNA', 'MOVE_CLASH_UP', 'NASTRAN', 'PACKAGED_ZIP', 'PRINCIPAL_AXES', 'RADIOSS', 'REMOVE_FROM_SETS', 'REMOVE_INCLUDE_ONLY', 'REMOVE_JUNIOR', 'RENUMBER_TO_FREE', 'SHIFT_ALL_UP', 'USER_DEFINED_CENTRE', 'WRITE_DIALOGUE', 'WRITE_INCLUDE_TREE', 'WRITE_MODEL', 'WRITE_SELECT_INCLUDES'}
    _props = {'highest'}
...
# Static methods
    ...
    def First():
        ...
        (or null if there are no models)
    ...
    def Merge(master_model, slave_model, option_to_fix_clashes=Oasys.gRPC.defaultArg, merge_nodes_flag=Oasys.gRPC.defaultArg, dist=Oasys.gRPC.defaultArg, label=Oasys.gRPC.defaultArg, position=Oasys.gRPC.defaultArg):
        """
        ...
        ----------
        master_model : Model
            Master Model for merge
        slave_model : Model
            Slave Model for merge
        option_to_fix_clashes : constant
            Type of fix. Can be Model.INCREASE_SLAVE_ALWAYS,
            Model.INCREASE_SLAVE_CLASH,
            Model.DISCARD_SLAVE_CLASH,
            Model.INCREASE_MASTER_ALWAYS,
            Model.INCREASE_MASTER_CLASH or
        ...
        """
        return Oasys.PRIMER._connection.classMethod(__class__.__name__, "Merge", master_model, slave_model, option_to_fix_clashes, merge_nodes_flag, dist, label, position)
    ...
    def Select(prompt, modal=Oasys.gRPC.defaultArg):
        ...
        If there are no models in memory then Select returns null. If only one model is present
...
# Instance methods
    ...
    def DeleteInclude(self, include_label, method=Oasys.gRPC.defaultArg, force=Oasys.gRPC.defaultArg):
        """
        ...
        """
        return Oasys.PRIMER._connection.instanceMethod(self.__class__.__name__, self._handle, "DeleteInclude", include_label, method, force)
