...
def multigrid_nonlin_3(forceu, u_in, prm):
    ...
    for i in range(nlevel):
        ...
        if i < nlevel - 1 and multigrid[i] > multigrid[i + 1]:
            ...
            prmin['h'] = h[l]
            if prm['nudim'] == 2:
                u[l] = navlam_nonlinear_2(forceu[l], v[l], prmin)
            elif prm['nudim'] == 3:
                u[l] = navlam_nonlinear_3(forceu[l], v[l], prmin)
            v[l] = u[l]
...
def navlam_nonlinear_2(forceu, u_in, prm):
    """
    Fix point iterations (isolating the unknown on left hand side and
    iterating). See page 100 in 'A multigrid tutorial'
    """
...
def navlam_nonlinear_3(forceu, u_in, prm):
    """
    Fix point iterations (isolating the unknown on left hand side and
    iterating). See page 100 in 'A multigrid tutorial'
    """
    ...
    F0 = np.zeros_like(u[0], dtype=DTYPE)
    F1 = np.zeros_like(u[1], dtype=DTYPE)
    F2 = np.zeros_like(u[2], dtype=DTYPE)
    temp = np.zeros_like(u[0], dtype=DTYPE)
