...
class Validation:
                ...
                for f, b in self.errortypes.items():
                    ...
                    if "indexrange" not in b["identificationprams"].keys():
                        ...
                        try:
                            ...
                            raise ValueError(f"please remopve back to quntum simbole or use re.escape in errortypes regex{validationregex}")
                        if len(vadatedata) > 0:
    ...
    def get_formetted_data(self, newdf=None):
        if newdf is None:
            ...
            return self.fillnewdf(rew, newdf)
        else:
            rew = self.get_error_types(newdf)
            return self.fillnewdf(rew, newdf)

    def fillnewdf(self, rew, newdf):
        if rew != []:
            for s, x in enumerate(rew):
                validateddata = self.get_identified_data(x)
                if newdf.empty:
                    newdf = pd.DataFrame(columns=list(validateddata["data"].keys()))
                    newdf.loc[s] = list(validateddata["data"].values())
                else:
                    newdf.loc[newdf.index[-1] + s] = list(
                        validateddata["data"].values()
                    )
        return newdf

    def get_identified_data(self, data):
        x = data
        newdata = {}
        if x["errortypes"] == 1:
            ...
            newdata["Tax Details"] = "".join(x["data"]["Tax Details"].split(" ")[1:])
            x["data"].update(newdata)
        elif x["errortypes"] == 6:
            ...
            newdata["Article Description"] = x["data"]["Artical"] + x["data"]["Artical"]
            newdata["HSN"] = "#{}".format(
                x["data"]["Article Description"].split("#")[1]
            )
            newdata["EAN"] = "#{}".format(
            ...
            newdata["MRP"] = x["data"]["MRP"]
            newdata["Cost"] = x["data"]["MRP"]
            newdata["Line Cost Excl Tax"] = x["data"]["Line Cost Excl Tax"].split(" ")[
                0
            ]
            newdata["Tax Details"] = "IN: {} - IN: GST Comp. CESS(0.00%) - 0.00".format(
                x["data"]["Tax Details"].split("IN")[0]
            )
            x["data"].update(newdata)
        elif x["errortypes"] == 13:
            newdata["Sr.No"] = x["data"]["Sr.No"]
            newdata["Artical"] = x["data"]["Artical"]
            newdata["Article Description"] = x["data"]["Article Description"]
            newdata["HSN"] = x["data"]["HSN"]
            ...
            newdata["Pack"] = x["data"]["Pack"]
            newdata["MRP"] = x["data"]["MRP"]
            newdata["Cost"] = x["data"]["Cost"].split("IN")[0][
                0 : int(len(x["data"]["Cost"].split("IN")[0]) / 2)
            ]
            newdata["Line Cost Excl Tax"] = x["data"]["Cost"].split("IN")[0]
            newdata["Tax Details"] = "IN: {} - IN: GST Comp. CESS(0.00%) - 0.00".format(
                x["data"]["Tax Details"].split("IN")[0]
            )
            x["data"].update(newdata)
    ...
    def datafillerdict(self, records, columns, ied, mj):
        ...
        for dwq in columns:
            ...
            if dwq == "HSN":
                reg = r"^#\d+$"
                reg=re.compile(reg)
                valo = reg.findall(records[columns.index(dwq)])
                if len(valo) > 0:
                    dictdata[dwq] = records[columns.index(dwq)]
                else:
                ...
                reg = r"^#\d{13}+$"
                reg=re.compile(reg)
                valo3 = reg.findall(records[columns.index(dwq) + 1])
                if len(valo3) > 0:
                    dictdata[columns[columns.index(dwq)]] = records[
                        columns.index(dwq) + 1
                    ]
                ...
                validationMRP=re.compile(validationMRP)
                rewq = validationMRP.findall(records[columns.index(dwq) + 1])
                if len(rewq) > 0:
                    dictdata[columns[columns.index(dwq)]] = records[
                        columns.index(dwq) + 1
                    ]
                ...
                if len(rewq) > 0:
                    ...
                    valos=re.compile(valos)
                    valos = valos.findall(records[columns.index(dwq) + 2])
                    if len(valos) == 0:
                        dictdata[columns[columns.index(dwq)]] = records[
                            columns.index(dwq)
                        ]
                ...
                valod=re.compile(valod)
                valis = valod.findall(dictdata[columns[columns.index(dwq)]])
                try:
