...
if "set_list_click" not in st.session_state:
    st.session_state["set_list_click"] = False

if "del_list_click" not in st.session_state:
    st.session_state["del_list_click"] = False

if "item_gotten" not in st.session_state:
    st.session_state["item_gotten"] = None
...
def get_one_item():
    with st.session_state["get_cookie_placement"][0]:
        localS.getItem(st.session_state["get_storage_item"], key="test_get_item_2")
    # st.write(get_item)
...
if "test_get_item_2" in st.session_state:
...
def getListFromStorage(): 
    st.session_state["set_list_click"] = not st.session_state["set_list_click"]
...
with cols[1].form("get_all_storage"):
   ...
   # st.multiselect("keys", options=["One", "two", "three", "four", "five", "six", "seven"], key="get_list_of_storage")
    st.form_submit_button("Get all storage", on_click=getListFromStorage)
    st.session_state["get_storage_list_placement"] = st.columns(1)


if st.session_state["set_list_click"]:
