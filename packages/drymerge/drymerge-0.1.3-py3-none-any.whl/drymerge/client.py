...
class DryClient:
    def __init__(self, api_key, engine_host: str = "https://api.drymerge.com", proxy_host="https://proxy-srv.drymerge.com",
    ...
    async def process_job(self, job):
        ...
        if handler:
            result = handler(job.request['body'])  # Ensure your handlers are async if necessary
            job_response = result
        else:
            self.report(f"[PROXY] No handler for job {job.id} with details {job.request}, skipping.")
            job_response = {"error": "No handler available for this job."}
...
#     verbose=True, engine_host="http://localhost:3000")
#           .route("print_with_reflect", lambda request: {'j1': 'Job Result!!! Hello!'})
#           .route("py2", lambda request: {'j2': 'ANOTHER RESULT'}))
#
# client.run(DryId("google-trigger-schema", type_="api"), args={"user_name": "<customer-user-name>", "refresh_token": "user-refresh-token"})
# # client.start()
#
