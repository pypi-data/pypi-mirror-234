...
from .process_log import sub_train_statues, sub_train_end, validate_statues ,validate_end
...
def train_meta_epoch(args, epoch, data_loader, encoder, decoders, optimizer):
    ...
    for sub_epoch in range(args.sub_epochs):
        ...
        for i, (data) in enumerate(data_loader):
            # log
            process_log.sub_train_statues(epoch,sub_epoch,i,len(data_loader))
        ...
        process_log.sub_train_end(epoch,sub_epoch,mean_loss)
...
def validate(args, epoch, data_loader, encoder, decoders):
    ...
    with torch.no_grad():
        for i, (image, label, mask, file_name, img_type) in enumerate(data_loader):
            process_log.validate_statues(i,len(data_loader))
    ...
    process_log.validate_end(epoch,mean_loss,img_auc,pix_auc)
