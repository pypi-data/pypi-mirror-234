...
def xnet2igraph(fileName='test.xnet',compressed=False):
    ...
    with myopen(fileName, myopenType,encoding="utf-8") as fp:
        currentLineIndex = 0
        lastLine = ""
...
def igraph2xnet(g,fileName='test.xnet',ignoredNodeAtts=[],ignoredEdgeAtts=[],compressed=False):
    ...
    fd = myopen(fileName,myopenType, encoding="utf-8")
        ...
        if att not in ignoredNodeAtts:
            ...
            for i in range(N):
                if typeSampleString == 'n':
                    ...
                    # escape characters
                    stringValue = g.vs[i][att].replace("\n","\\n")
                    fd.write('\"'+stringValue+'\"'+'\n')
        ...
        if att not in ignoredEdgeAtts:
            ...
            for i in range(E):
                if typeSampleString == 'n':
                    ...
                    stringValue = g.es[i][att].replace("\n","\\n")
                    fd.write('\"'+stringValue+'\"'+'\n')				
