...
class RelationalAlgebraFrozenSet(abc.RelationalAlgebraFrozenSet):
    ...
    def _drop_duplicates(container):
        ...
        if duckdb and container.dtypes.dtype not in container.dtypes:
            columns = container.columns
            con = duckdb.connect()
            container = con.execute("SELECT DISTINCT * FROM container").df()
...
class NamedRelationalAlgebraFrozenSet(
    ...
    def naturaljoin(self, other):
        ...
        if duckdb:
            ...
            con.register("left_", left)
            con.register("right_", right)
            new_container = con.execute("SELECT DISTINCT * FROM left_ NATURAL INNER JOIN right_").df()
            ...
            might_have_duplicates = self._might_have_duplicates | other._might_have_duplicates
        return self._light_init_same_structure(
            new_container,
