...
class RabbitMQManager:
    ...
    def _init_producer_channel(self, retry=0):
        try:
            ...
            threading.Thread(target=self._init_producer_channel, kwargs={"retry": retry}).start()
    ...
    def error_stack_rabbitmq(self, message=None, retry=0, error_location=None):
        try:
            ...
            if self.is_retry_producer_connection == False:
                ...
                threading.Thread(target=self._init_producer_channel).start()
    ...
    def start_subscription(self, callback=None):
        ...
        self._init_producer_channel()
        threading.Thread(target=self._subscription, kwargs={"callback": callback}).start()

    def _subscription(self, callback=None, retry=0):
        try:
            connection = pika.BlockingConnection(pika.ConnectionParameters(host=self.host, port=self.port, virtual_host=self.vhost, credentials=self.credential))
            ...
            if self.is_stopped == True:
                ...
                time.sleep(30)
                threading.Thread(target=self._subscription, kwargs={"callback": callback, "retry": retry}).start()
