...
class ProphecyDebugger:
    ...
    def find_current_pipeline_code_in_path(cls, pipeline_uri):
        wheels = ProphecyDebugger.wheels_in_path() + ProphecyDebugger.wheels_in_site_packages()
        for wheel_path in wheels:
            possible_content = ProphecyDebugger.find_pipeline_json_in_wheel(wheel_path, pipeline_uri)
            if possible_content:
                return possible_content
        ProphecyDebugger.log(None, f"Could not find pipeline code for pipeline {pipeline_uri} in wheels {wheels}")
        return None
