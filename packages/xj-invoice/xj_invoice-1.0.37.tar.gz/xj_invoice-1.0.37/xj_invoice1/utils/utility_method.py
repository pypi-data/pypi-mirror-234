...
# 字符串转列表
# 1. 如果传入的是字符串，它会检查是否有逗号，然后将其分割成列表或者直接转为包含一个元素的列表。
# 2. 如果传入的是整数，它会将整数转换成字符串，并放入一个列表中。
# 3. 如果传入的是列表，它会将列表中的每个元素转换成字符串。
def parse_integers(value):
    if isinstance(value, str):
        ...
        lst = [str(num) for num in value]
    else:
        raise ValueError("不支持的值类型。应为字符串、int或list")
...
# def build_tree(data, parent_key=None, child_key=None):
...
#
#     for item in data:
#         nodes[item[child_key]] = item
#
#     for item in data:
#         parent_id = item[parent_key]
#         if parent_id == None or parent_id not in nodes:
#             tree.append(item)
#         else:
#             parent_item = nodes[parent_id]
#             parent_item.setdefault('children', []).append(item)
#
#     return list(filter(lambda x: x['parent'] == None, tree))

def build_tree(data, parent_key=None, child_key=None, extra_keys=None):
    if parent_key is None or child_key is None:
        return None
    ...
    # 找到根节点
    root_id = df[df[parent_key].isnull()].iloc[0][child_key]
    ...
    for index, row in df.iterrows():
        node = {k: row[k] for k in df.columns if k in row}
        for key, value in node.items():
            if pd.isnull(value) or value == "":
                node[key] = None
            elif isinstance(value, float):
                node[key] = int(value)
        node_dict[row[child_key]] = node

    # 构建树结构
    def build_node(parent_id):
        node = node_dict[parent_id]
        children = []
        for child_id, child_node in node_dict.items():
            if child_node[parent_key] == parent_id:
...
def custom_sort(data, reverse=False, **kwargs):
    """
    通用排序函数，可以按照指定规则排序列表。

    参数:
    data (list): 待排序的列表。
    reverse (bool): 是否进行倒序排序，默认为 False。
    kwargs: 其他用于提取排序字段的参数。
    ...
    # 定义提取排序字段的函数
    def sort_key(item):
        # 根据参数 kwargs 在 item 中提取相应的字段，并返回用于排序的键
        return tuple(item[key] for key in kwargs.values())
