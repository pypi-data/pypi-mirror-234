...
class OperationProducer(BaseProducer):
    ...
    def _get_analytical_accounts(self, line) -> List[dict]:
        ...
        Cas 3
        15/11/2021 BQ 101000 "2" "TVA" C 6000,00 E  A ( axe1 COOP00 1000 axe1 COOP01 5000 )
        """
        # Join analytical related lines
        end_columns = " ".join(line[9:])
        if "(" in end_columns and ")" in end_columns:
            analytical_columns = end_columns.split("(")[1].split(")")[0].split(" ")
        else:
            analytical_columns = []
                    ...
                    analytical_accounts.append(
                        {
                            ...
                            "value": convert_to_float(value.strip()),
                        }
                    )
        else:
            # Should not happen
