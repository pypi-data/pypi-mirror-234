...
from dotcoder import prompts, voice
...
class Commands:
    ...
    def cmd_undo(self, args):
        "Undo the last git commit if it was done by dotcoder"
        if not self.coder.repo:
            self.io.tool_error("No git repository found.")
            return
        ...
        if (
            not last_commit.message.startswith("dotcoder:")
            or last_commit.hexsha[:7] != self.coder.last_dotcoder_commit_hash
        ):
            self.io.tool_error("The last commit was not made by dotcoder in this chat session.")
            return
        self.coder.repo.repo.git.reset("--hard", "HEAD~1")
        self.io.tool_output(
            f"{last_commit.message.strip()}\n"
            f"The above commit {self.coder.last_dotcoder_commit_hash} "
    ...
    def cmd_diff(self, args):
        "Display the diff of the last dotcoder commit"
        if not self.coder.repo:
            self.io.tool_error("No git repository found.")
            return

        if not self.coder.last_dotcoder_commit_hash:
            self.io.tool_error("No previous dotcoder commit found.")
            return

        commits = f"{self.coder.last_dotcoder_commit_hash}~1"
        diff = self.coder.repo.diff_commits(
            self.coder.pretty,
            commits,
            self.coder.last_dotcoder_commit_hash,
        )
    ...
    def cmd_add(self, args):
        ...
        if self.coder.repo and git_added:
            ...
            commit_message = f"dotcoder: Added {git_added}"
            self.coder.repo.commit(message=commit_message)
