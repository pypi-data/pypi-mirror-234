...
class OAIWithOpenAI4:
    ...
    def invoke(self,df2,df1,sample):
        
        oai = OpenAIEmbedding(self.key)
        df = pd.read_json(oai.invoke(df1,df2))
        print(df)
        # split df into parts on the basis of the column length of df1 and df2
        table1_parts = df.iloc[:, :len(df1.columns)]
        table2_parts = df.iloc[:, len(df1.columns):]
        
        #split df into parts as per split size
        table1_parts = np.array_split(table1_parts,OAIWithOpenAI4.__get_split_size(len(table1_parts)))
        table2_parts = np.array_split(table2_parts, OAIWithOpenAI4.__get_split_size(len(table2_parts)))
        print("t1\n",table1_parts)
        print("t2\n",table2_parts)
