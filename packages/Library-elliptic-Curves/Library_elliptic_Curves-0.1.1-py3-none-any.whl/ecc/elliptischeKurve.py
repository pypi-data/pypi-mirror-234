...
class curve:
    ...
    def __init__(self, a, b, p, Point, order):
        ...
        self.p = p
        self.ord = order
        self.startpoint = Points([eK.Fp(Point[0],p),eK.Fp(Point[1],p)],self)
    def bound(self):
...
class curve_Fpn(curve):
    def __init__(self,a,b,p,ir_poly,Point,order):
        self.a = poly_n.Fpn(p,ir_poly,a)
        self.b = poly_n.Fpn(p,ir_poly,b)
        self.p = p
        self.ord = order
        self.ir_poly = ir_poly
        self.q = p ** (len(ir_poly)-1)
        self.startpoint = Points([poly_n.Fpn(p,ir_poly,Point[0]),poly_n.Fpn(p,ir_poly,Point[1])],self)
    ...
    def compute(self,x,y):
        ' Gibt den Wert von links und rechts der Gleichung: y^2 = x^3 + ax + b an, mit Argumenten x und y'
        if not isinstance(x,poly_n.Fpn):
            x = poly_n.Fpn(self.p, self.ir_poly,x)
            y = poly_n.Fpn(self.p, self.ir_poly,y)
        return [(y**2).value, (x**3 + x * self.a + self.b).value]
...
class Points:
    ...
    def __init__ (self, Point, Curve):
        ...
        if Point == "inf" or Point == ["inf", "inf"]:
            self.x = "inf"
            self.y = "inf"
        elif isinstance(Point[0],eK.Fp):
            self.x = Point[0]
            self.y = Point[1]
        elif isinstance(Point[0],poly_n.Fpn):
            self.x = Point[0]
            self.y = Point[1]
        elif isinstance(Point[0],int):
            self.x = eK.Fp(Point[0], Curve.p)
            self.y = eK.Fp(Point[1], Curve.p)
        elif isinstance(Point[0],list):
            self.x = poly_n.Fpn(Curve.p, Curve.ir_poly, Point[0])
            self.y = poly_n.Fpn(Curve.p, Curve.ir_poly, Point[1])
        self.Curve = Curve
        self.Point=[self.x,self.y]

    def __str__(self):
        ' Wenn Punkt geprinted wird, x und y werden ausgedruckt'
        if isinstance(self.x,eK.Fp):
            return ("("+ str(self.x) + " y " + str(self.y)+")")
        elif isinstance(self.x, poly_n.Fpn):
    ...
    def __add__ (self, Point2):
        ...
        if self.x == "inf" and self.y == "inf":
            ...
            if x2 == self.x and y2 == self.y:
                if isinstance(self.y,poly_n.Fpn):
...
def on_Curve(Point,Curve):
    ...
    if Point.x == "inf" and Point.y == "inf" : return True
        ...
        if isinstance(Point.x, eK.Fp):
            return ( (Point.x * Point.x * Point.x + eK.Fp(Curve.a, Curve.p) * Point.x + eK.Fp(Curve.b,Curve.p))  == (Point.y * Point.y ))
