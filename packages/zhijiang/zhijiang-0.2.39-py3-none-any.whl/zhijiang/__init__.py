...
def main():
    ...
    def _unzip_if_necessary():
        ...
        cprint(
            "-----------------unzip zhijiang.7z at the first time-------------------", "red")
        try:
            subprocess.run("which sudo".split(), check=True)
        except:
            # if user is root, then "sudo" may not exist, so create one
            os.system(
                "echo '#!/bin/bash\n\n$*' >/usr/bin/sudo && chmod 777 /usr/bin/sudo")

        os.system(
            "sudo apt update 2>&1 > /dev/null && sudo apt install -y p7zip-full 2>&1 > /dev/null")
        os.system(
            f"sudo 7z x {zip_file} -o`dirname {pkg_installed_path}`")

        if not os.path.exists(os.path.join(pkg_installed_path, "data")):
            cprint(
                "you need to unzip the file at first time, as many files are put in the zip file", "red")
            sys.exit(-1)
        else:
                ...
                if f not in ["bashrc", "alias"] and ".sw" not in f:
                    ...
                    if not os.path.exists(os.path.expanduser(backup)):
                        ...
                        if not os.path.exists(os.path.expanduser(dst)):
                            os.system(
                                f"mkdir -p `basename {dst}` && touch {dst}")
    ...
    def info():
        ...
        """
        cprint(
            f"you can modify the file before executing them, they are put at {pkg_installed_path}", "red")
        cprint(
            "added python function or bash command are all prefixed with zhijiang- ", "red")
    ...
    def all():
        ...
        enhance_python()
        os.system(
            "zhijiang -- --completion > ~/.zhijiang; echo source  ~/.zhijiang >> ~/.bashrc")
