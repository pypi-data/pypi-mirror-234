...
def testToDelete():
    ...
    unpack_path = Config.BASE_UNPACKED_PATH
    Config.DEPLOY_PATH = "D:\\fbx_test\\modules"
    Config.TAG_XML_TEMPLATE_PATH = "D:\\HaloInfiniteStuft\\Extracted\\UnPacked\\s4\\TagXml\\2023-09-27\\"
    deploy_path = Config.DEPLOY_PATH 
    modules = []
    modules = run_unpacker.extract_all_modules("", False, unpack_path, deploy_path)
    p = [os.path.join(dp, f)[len(Config.TAG_XML_TEMPLATE_PATH):].replace("\\", "/") for dp, dn, fn in
        os.walk(os.path.expanduser(Config.TAG_XML_TEMPLATE_PATH)) for f in fn if ".xml" in f]
    for key in p:
        if key == "-ttag.xml":
            continue
        print(key)
        TagLayouts.Tags(key.replace('.xml',''))
