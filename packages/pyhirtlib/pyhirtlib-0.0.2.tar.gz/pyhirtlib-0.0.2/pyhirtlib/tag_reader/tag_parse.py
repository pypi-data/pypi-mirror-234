...
from tag_reader.tag_file import RefItCount, TagFile
...
class TagParse:
    ...
    def onEntryRead(self, f, entry: TagStruct):
        if not (entry is None):
            ...
            if entry.type_id_tg != TagStructType.Root:
                ...
                if (tag.ElemntType() == TagElemntType.Struct):
            ...
            if self.debug:
                if tag.ElemntType() == TagElemntType.RootTagInstance:
    ...
    def readTagDefinition(self,f, i, k, entry: TagStruct, tags: TagLayouts.C, outresult: TagFileMap, field_offset:int = 0) -> int:
        ...
        for address in tags.B:
            ...
            if child_tag.ElemntType() == TagElemntType.Struct:
                self.readTagDefinition(f, i, k, entry, child_tag, outresult, field_offset + address)
            elif child_tag.ElemntType() == TagElemntType.Array:
    ...
    def verifyAndAddTagBlocks(self, tag_maps: TagFileMap, child_item: TagLayouts.C, field_offset: int):
        if child_item.ElemntType() == TagElemntType.DataV2:
            tag_maps.datas[field_offset] = child_item
            return
        elif child_item.ElemntType() == TagElemntType.TagReference:
            tag_maps.refers[field_offset] = child_item
            return
        elif child_item.ElemntType() == TagElemntType.Struct:
            if child_item.E["comp"] == "1" : 
                tag_maps.blocks[field_offset] =  child_item
            return
        elif child_item.ElemntType() == TagElemntType.Block:
            tag_maps.blocks[field_offset] =  child_item
            return
        elif child_item.ElemntType() == TagElemntType.ResourceHandle:
            tag_maps.blocks[field_offset] =  child_item
            return
        else:
            return