...
class ValueCounts:
    def exec(self, flow_id, node_key, pin, settings):
        ...
        try:
            df = df.value_counts(
                subset=field, normalize=normalize, ascending=ascending, dropna=drop_na
            )
            df = df.reset_index(name="value_count")
            script.append(
                "df = df.value_counts(subset=[{}], normalize={}, ascending={}, dropna={})".format(
                    field, normalize, ascending, drop_na
                )
            )
            script.append("df = df.reset_index(name='value_count')")
