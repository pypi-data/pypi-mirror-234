...
class DatetimeExtract:
    ...
    def exec(self, flow_id, node_key, pin, settings):
        ...
        column_to_convert: str = (
            ...
            if ("column_to_convert" in settings and settings["column_to_convert"] is not None)
            else None
        )
        to_extract: str = (
            settings["to_extract"]
            if ("to_extract" in settings and settings["to_extract"] is not None)
            else None
        )
        new_column_name: str = (
            settings["new_column_name"]
            if (
                "new_column_name" in settings
                and settings["new_column_name"] is not None
                and settings["new_column_name"] != ""
            )
