...
class Builder:
    ...
    def analyzer(
        ...
        while num_nodes_ok < len(self.nodes):
            ...
            for node_key, node_data in self.nodes.items():
                ...
                if disable_all_write_nodes and node_data["type"] == NodeType.OUTPUTDATA.value:
                ...
                if is_loaded:
                    ...
                    # --------------------------------------------------
                    # same_config = 'config' in cache_handler.cache[flow_id][node_key] and cache_handler.cache[flow_id][node_key]['config'] == json.dumps(data['nodeDataArray'][idx]['meta']['config'], sort_keys=True)
                    node_cache = cache_handler.get_node(flow_id, node_key)
                    if (
                        node_cache
                        and "config" in node_cache
                        and node_cache["config"]
                        == json.dumps(
    ...
    def modify_node(self, flow_id: str, node_key: str, node: dict):
        cached_node = cache_handler.get_node(flow_id, node_key)
        if cached_node:
            # flow[node_key] = node
            pout = cached_node["pout"]
            for port_name in node["meta"]["ports_map"]["pout"].keys():
                node["meta"]["ports_map"]["pout"][port_name][
                    "head"
                ] = utilities.get_head_of_df_as_list(
                    pout[port_name], self.get_table_config(node["meta"])
                )
        return {"flow_id": flow_id, "node_key": node_key, "node": node}

    def get_table_config(self, meta: dict):
        return {
